const KOTERET_CLIENT_BUILD = "205.0.0";
...
function init() {
  ...
  // V205: never paint a device-local lead headline during first entry. The feed
...
async function verifyApiVersion() {
  ...
  try {
    ...
    marker.textContent = apiVersion ? `גרסה V205 · API ${apiVersion}` : "גרסה V205 · API לא מזוהה";
  } catch (error) {
    marker.textContent = "גרסה V205 · API לא מחובר";
...
async function registerServiceWorker() {
  if (!("serviceWorker" in navigator)) return;
  try {
    state.serviceWorkerRegistration = await navigator.serviceWorker.register("/sw.js?v=205.0.0", { updateViaCache: "none" });
...
async function getReadyPushServiceWorkerRegistration() {
  ...
  if (!registration) {
    registration = await navigator.serviceWorker.register("/sw.js?v=205.0.0", { updateViaCache: "none" });
