...
async function serveEscalationHtmlAsset(request, env, origin) {
  ...
  const html = (await asset.text())
    .replaceAll("__SITE_URL__", PUBLIC_SITE_ORIGIN)
    ...
    try {
      response = await fetch(articleUrl, {
        ...
        headers: {
          ...
          "User-Agent": "Mozilla/5.0 (compatible; KoteretPlus/151; +https://koteretplus.com/)"
        }
      });
...
// V151 background calibration refresh: split the external OSINT set into three
...
async function handleEscalation(request,env,ctx){
  try{
    ...
    if(!claim?.claimed&&claim?.public?.latest)return json(claim.public,200,{"Cache-Control":"no-store","X-Hadashota-Version":"151.0.0"});
    if(!claim?.claimed){const p=await escalationHubCall(env,"/escalation/public");return json(p,200,{"Cache-Control":"no-store","X-Hadashota-Version":"151.0.0"});}
    ...
    const publicData=await escalationHubCall(env,"/escalation/snapshot","POST",payload);return json(publicData,200,{"Cache-Control":"no-store","X-Hadashota-Version":"151.0.0"});
  }catch(error){console.warn("Escalation refresh failed",error);try{const p=await escalationHubCall(env,"/escalation/public");return json({...p,refreshError:String(error?.message||error)},200,{"Cache-Control":"no-store","X-Hadashota-Version":"151.0.0"});}catch{return json({ok:false,error:"Escalation index temporarily unavailable"},503,{"Cache-Control":"no-store"});}}
}
...
async function refreshBackgroundNewsShards(env,ctx,shards=[]){
  ...
  await Promise.all(shards.map(async(shard)=>{
    ...
    }catch(error){console.warn(`V151 background shard ${shard} failed`,error);}
  }));
  return fresh;
}
...
async function runBackgroundEscalationFromNews(env,recent=[]) {
  ...
  }catch(error){console.warn("V151 scheduled escalation monitor failed",error);}
}
...
async function runBackgroundPushMonitor(env, ctx) {
  ...
  try {
    ...
    console.warn("V151 scheduled push monitor failed",error);
  }
}
...
async function ensurePushStats(storage) {
  ...
  if(stats?.schema==="v151"&&Number.isFinite(Number(stats.count)))return stats;
  const rows=await storage.list({prefix:"sub:"});
  // One-time V151 repair: a browser can rotate its PushSubscription endpoint.
  ...
  stats={schema:"v151",count:cleanRows.size,platforms,updatedAt:new Date().toISOString(),deduplicated:remove.length};
...
async function queuePushJob(storage,notification,targetDeviceId="") {
  ...
  if(active){
    ...
    await storage.put("push.job",{...queued,startedAt:now,cursor:"",processed:0,pushed:0,removed:0,failed:0,expected:targetKey?1:Number(stats.count||0)});
...
async function startNextPushJob(storage) {
  ...
  const stats=await ensurePushStats(storage);
  await storage.put("push.job",{...next,startedAt:new Date().toISOString(),cursor:"",processed:0,pushed:0,removed:0,failed:0,expected:next.targetKey?1:Number(stats.count||0)});
...
async function finalizePushJob(storage,job) {
  ...
  if(i>=0)history[i]={...history[i],status:"done",finishedAt:result.finishedAt,pushed:result.pushed,failed:result.failed,removed:result.removed};
  await storage.put("push.history",history.slice(-30));
...
export class PushHub {
  ...
  async fetch(request){
    ...
    if(url.pathname==="/config"){
      ...
      const stats=await ensurePushStats(storage);
      return json({enabled:true,publicKey:keys.publicKey,subscriptions:Number(stats.count||0),platforms:stats.platforms||{},fanout:"paged-alarm",mode:"true-web-push",version:"151.0.0"},200,{"Cache-Control":"no-store"});
    }
    ...
    if(url.pathname==="/subscribe"&&request.method==="POST"){
      ...
      if(deviceId){
        ...
        if(previousKey&&previousKey!==key){
          ...
          if(previousRow){await storage.delete(previousKey);stats=await updatePushStats(storage,-1,previousRow.platform||pushPlatformFromUserAgent(previousRow.userAgent));}
        }
      }
      ...
      await storage.put(key,{subscription,deviceId,platform,userAgent,createdAt:existing?.createdAt||now,lastSeenAt:now,pendingFingerprint:existing?.pendingFingerprint||""});
    ...
    if(url.pathname==="/status"){
      ...
      return json({enabled:true,subscriptions:Number(stats.count||0),platforms:stats.platforms||{},lastPushedFingerprint:previous||null,latest:latest||null,fanoutActive:!!activeJob,lastResult:lastResult||null,version:"151.0.0"},200,{"Cache-Control":"no-store"});
    }
    ...
    if(url.pathname==="/lead"&&request.method==="POST"){
      ...
      if(!last){await storage.put("lead.lastPushedFingerprint",payload.fingerprint);await storage.delete("lead.candidate");return json({ok:true,primed:true,pushed:0});}
      if(last===payload.fingerprint){await storage.delete("lead.candidate");return json({ok:true,changed:false,pushed:0});}
  ...
  async alarm(){
    ...
    for(let i=0;i<rows.length;i+=5){
      ...
      const prepared=await Promise.all(batch.map(async([key,row])=>{const updated={...row,pendingFingerprint:job.notificationFingerprint,pendingAt:new Date().toISOString()};await storage.put(key,updated);return [key,updated];}));
      const results=await Promise.all(prepared.map(async([key,row])=>{
