...
async function handleEmergencyAlerts(ctx) {
  ...
  const cacheKey = new Request("https://hadashota.internal/v94/oref-current", { method: "GET" });
...
async function findCommonsMedia(query, specificity = 1) {
  ...
  try {
    ...
    const threshold = subjectStrict
      ? (specificity >= 3 ? 28 : specificity >= 2 ? 25 : 22)
      : (specificity >= 3 ? 34 : specificity >= 2 ? 30 : specificity >= 1 ? 24 : 70);
...
async function handleOpenMedia(url, ctx) {
  ...
  const cache = caches.default;
  const cacheKey = new Request(`https://hadashota.media.local/v94-commons-broader?q=${encodeURIComponent(raw)}&c=${encodeURIComponent(category)}`);
  const cached = await cache.match(cacheKey);
  if (cached) return cors(cached);
