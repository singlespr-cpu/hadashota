...
async function handleEmergencyAlerts(ctx) {
  ...
  const cacheKey = new Request("https://hadashota.internal/v112/oref-current", { method: "GET" });
...
async function handleOpenMedia(url, ctx) {
  ...
  const cache = caches.default;
  const cacheKey = new Request(`https://hadashota.media.local/v112-instant-paint?q=${encodeURIComponent(raw)}&c=${encodeURIComponent(category)}`);
  const cached = await cache.match(cacheKey);
  if (cached) return cors(cached);
