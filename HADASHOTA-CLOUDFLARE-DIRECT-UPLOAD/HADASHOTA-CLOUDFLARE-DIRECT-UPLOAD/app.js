const KOTERET_CLIENT_BUILD = "151.0.0";
...
async function verifyApiVersion() {
  ...
  try {
    ...
    marker.textContent = apiVersion ? `גרסה V151 · API ${apiVersion}` : "גרסה V151 · API לא מזוהה";
  } catch (error) {
    marker.textContent = "גרסה V151 · API לא מחובר";
...
function maybeShowNotificationOffer() {
  ...
  if(iosNeedsInstall){
    ...
    if(el.notificationOfferAccept)el.notificationOfferAccept.textContent="איך מפעילים באייפון";
  ...
  if(el.notificationOfferAccept)el.notificationOfferAccept.textContent="הפעלת התראות";
  notificationOfferShownThisSession = true;
  openSiteModal(el.notificationOfferModal);
}
...
async function registerServiceWorker() {
  if (!("serviceWorker" in navigator)) return;
  try {
    state.serviceWorkerRegistration = await navigator.serviceWorker.register("/sw.js?v=151.0.0", { updateViaCache: "none" });
...
async function getReadyPushServiceWorkerRegistration() {
  ...
  if (!registration) {
    registration = await navigator.serviceWorker.register("/sw.js?v=151.0.0", { updateViaCache: "none" });
...
async function toggleHeadlineNotifications(desiredState = !state.notificationsEnabled) {
  ...
  if (!("Notification" in window) || !("serviceWorker" in navigator) || !("PushManager" in window)) {
    showToast("הדפדפן לא תומך ב-Push");
    return;
  }
