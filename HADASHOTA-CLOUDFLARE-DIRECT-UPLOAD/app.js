const KOTERET_CLIENT_BUILD = "177.0.0";
...
async function verifyApiVersion() {
  ...
  try {
    ...
    marker.textContent = apiVersion ? `גרסה V177 · API ${apiVersion}` : "גרסה V177 · API לא מזוהה";
  } catch (error) {
    marker.textContent = "גרסה V177 · API לא מחובר";
...
function renderFeed() {
  ...
  // V177: the sponsored rectangle lives directly after "האחרונים החשובים"
...
async function registerServiceWorker() {
  if (!("serviceWorker" in navigator)) return;
  try {
    state.serviceWorkerRegistration = await navigator.serviceWorker.register("/sw.js?v=177.0.0", { updateViaCache: "none" });
...
async function getReadyPushServiceWorkerRegistration() {
  ...
  if (!registration) {
    registration = await navigator.serviceWorker.register("/sw.js?v=177.0.0", { updateViaCache: "none" });
