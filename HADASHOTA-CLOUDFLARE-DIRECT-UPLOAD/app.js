const KOTERET_CLIENT_BUILD = "182.0.0";
...
async function verifyApiVersion() {
  ...
  try {
    ...
    marker.textContent = apiVersion ? `גרסה V182 · API ${apiVersion}` : "גרסה V182 · API לא מזוהה";
  } catch (error) {
    marker.textContent = "גרסה V182 · API לא מחובר";
...
function renderFeed() {
  ...
  // V182: the sponsored rectangle lives directly after "האחרונים החשובים"
...
async function registerServiceWorker() {
  if (!("serviceWorker" in navigator)) return;
  try {
    state.serviceWorkerRegistration = await navigator.serviceWorker.register("/sw.js?v=182.0.0", { updateViaCache: "none" });
...
async function getReadyPushServiceWorkerRegistration() {
  ...
  if (!registration) {
    registration = await navigator.serviceWorker.register("/sw.js?v=182.0.0", { updateViaCache: "none" });
