const KOTERET_CLIENT_BUILD = "196.0.0";
...
async function verifyApiVersion() {
  ...
  try {
    ...
    marker.textContent = apiVersion ? `גרסה V196 · API ${apiVersion}` : "גרסה V196 · API לא מזוהה";
  } catch (error) {
    marker.textContent = "גרסה V196 · API לא מחובר";
...
async function loadNews(force = false, fromRetry = false, silent = false) {
  ...
  try {
    ...
    // V196: every newsroom refresh (initial, manual and automatic) uses the
    ...
    if (materiallyChanged || !state.lastNewsFingerprint) {
      // V196: rendering is also cooperative. Most blocks are small, but yielding
...
async function registerServiceWorker() {
  if (!("serviceWorker" in navigator)) return;
  try {
    state.serviceWorkerRegistration = await navigator.serviceWorker.register("/sw.js?v=196.0.0", { updateViaCache: "none" });
...
async function getReadyPushServiceWorkerRegistration() {
  ...
  if (!registration) {
    registration = await navigator.serviceWorker.register("/sw.js?v=196.0.0", { updateViaCache: "none" });
