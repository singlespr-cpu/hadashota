const KOTERET_CLIENT_BUILD = "236.0.0";
...
const el = {
  ...
  pushOrefSettingsBtn: document.querySelector("#pushOrefSettingsBtn"),
  pushOrefSummary: document.querySelector("#pushOrefSummary"),
  ...
  mobileEscalationTop: document.querySelector("#mobileEscalationTop"),
  mobileTopEscalationScore: document.querySelector("#mobileTopEscalationScore"),
  mobileTopEscalationLevel: document.querySelector("#mobileTopEscalationLevel")
};
...
async function loadEscalationTeaser() {
  if (!el.escalationTeaser && !el.navEscalationScore && !el.desktopEscalationBox && !el.mobileEscalationTop) return;
  ...
  try {
    ...
    if (el.mobileTopEscalationScore) el.mobileTopEscalationScore.textContent = String(score);
    if (el.mobileTopEscalationLevel) el.mobileTopEscalationLevel.textContent = latest.level || "מדד פעיל";
    ...
    el.mobileEscalationTop?.setAttribute("data-level", String(latest.levelKey || "routine"));
  } catch (error) {
...
function init() {
  ...
  // Join the shared Worker snapshot on entry. A manual refresh remains the only
...
function serverFullDisplayEntry(data) {
  const raw = data?.displayEntry;
  if (!raw?.item || !raw?.fingerprint) return null;
  if (String(raw.fingerprint || "") !== String(data?.fingerprint || "")) return null;
  ...
  if (!Array.isArray(item.updates)) item.updates = Array.isArray(raw.reports) ? raw.reports : [];
  ...
  return {
    ...
    recentReports,
    uniqueSources,
    ageMinutes: Math.max(0, Number(raw.ageMinutes) || ageMs / 60_000),
    latestAt,
    qualificationAt: uniqueSources >= 3 ? leadQualificationAt(recentReports) : null,
    score: Number(raw.score) || 0,
    hotScore: Number(raw.hotScore) || storyHotScore(item),
...
async function hydrateInitialLeadFromServer({ allowWaitingState = false } = {}) {
  ...
  try {
    ...
    // V230: legacy local snapshots may contain a publisher image saved by an
...
async function verifyApiVersion() {
  ...
  try {
    ...
    marker.textContent = apiVersion ? `גרסה V236 · API ${apiVersion}` : "גרסה V236 · API לא מזוהה";
  } catch (error) {
    marker.textContent = "גרסה V236 · API לא מחובר";
...
function bindEvents() {
  ...
  el.feed?.addEventListener("click", (event) => {
    ...
    if(adLink){trackAdEvent(adLink.dataset.adSlot,{id:adLink.dataset.adId},"click");return;}
  ...
  el.alertAllIsrael?.addEventListener("change", () => {
    ...
    syncAlertSettings();
    syncPushPreferencesToServer().catch(()=>{});
  });
  ...
  el.notificationToggle?.addEventListener("change", async () => {
    await toggleHeadlineNotifications(el.notificationToggle.checked);
  });
  el.pushNewsToggle?.addEventListener("change", async () => {
    state.pushNews = el.pushNewsToggle.checked;
    localStorage.setItem("hadashota.pushNews", state.pushNews ? "1" : "0");
    await syncPushPreferencesToServer();
    syncControlsFromState();
  });
  el.pushEscalationToggle?.addEventListener("change", async () => {
  ...
  el.pushOrefToggle?.addEventListener("change", async () => {
    await setAlertDesktop(el.pushOrefToggle.checked);
    syncControlsFromState();
  });
  el.pushOrefSettingsBtn?.addEventListener("click", () => openSiteModal(el.alertSettingsModal, el.pushOrefSettingsBtn));
  ...
  el.notificationsBtn?.addEventListener("click", () => toggleHeadlineNotifications(!state.notificationsEnabled));
  el.shareSiteBtn?.addEventListener("click", shareKoteretPlus);
  el.quickNotifications?.addEventListener("click", () => toggleHeadlineNotifications(!state.notificationsEnabled));
  ...
  el.notificationOfferAccept?.addEventListener("click", async () => {
    if (isIOSDevice() && !isStandaloneMode()) {
      if (el.notificationOfferAccept.dataset.stage === "ios-guide") {
        closeSiteModal(el.notificationOfferModal, false);
        return;
      }
      if (el.notificationOfferTitle) el.notificationOfferTitle.textContent = "כך מפעילים Push באייפון";
      if (el.notificationOfferText) el.notificationOfferText.innerHTML = "<strong>1.</strong> לחצו שיתוף ↥ בדפדפן. <strong>2.</strong> בחרו ‘הוסף למסך הבית’. <strong>3.</strong> פתחו את כותרת פלוס מהאייקון שנוסף. <strong>4.</strong> לחצו שוב ‘הפעלת התראות’ ואשרו את חלון iOS.";
      el.notificationOfferAccept.dataset.stage = "ios-guide";
      el.notificationOfferAccept.textContent = "הבנתי";
      return;
    }
    el.notificationOfferAccept.disabled = true;
    el.notificationOfferAccept.textContent = "ממתין לאישור…";
    if (el.notificationOfferTitle) el.notificationOfferTitle.textContent = "אשרו עכשיו את ההתראות בדפדפן";
    if (el.notificationOfferText) el.notificationOfferText.innerHTML = "הדפדפן יציג בקשת הרשאה ליד שורת הכתובת. בחרו <strong>אפשר / Allow</strong>. אין לאתר אפשרות לעקוף את חלון האבטחה של הדפדפן.";
    ...
    el.notificationOfferAccept.disabled = false;
    el.notificationOfferAccept.textContent = "נסו שוב";
    if (result?.reason === "denied") {
      if (el.notificationOfferTitle) el.notificationOfferTitle.textContent = "הדפדפן חסם את ההתראות";
      if (el.notificationOfferText) el.notificationOfferText.innerHTML = "כדי לפתוח מחדש: לחצו על <strong>סמל המנעול / הגדרות האתר</strong> ליד כתובת האתר → <strong>התראות</strong> → <strong>אפשר</strong>, ואז נסו שוב.";
    } else if (el.notificationOfferText) {
      el.notificationOfferText.textContent = result?.message || "לא הצלחנו להשלים את החיבור. נסו שוב בעוד רגע.";
    }
  });
  ...
  document.querySelectorAll('[data-open-contact]').forEach((link) => {
    link.addEventListener("click", (event) => {
      event.preventDefault();
      const trigger = link.closest("#aboutModal") ? el.aboutBtn : link;
      openContactModal(link.dataset.contactTopic || "", trigger);
    });
  });
...
function maybeShowNotificationOffer() {
  ...
  const lastPromptAt=Number(localStorage.getItem("hadashota.notificationPromptLastShownAt")||0);
  if(lastPromptAt && Date.now()-lastPromptAt < 30*60*1000)return;
  const choice=localStorage.getItem("hadashota.notificationPromptChoice");
  const iosNeedsInstall=isIOSDevice()&&!isStandaloneMode();

  // iPhone/iPad Safari does not expose the Web Push APIs to a normal browser
  // tab. Still show the first-visit offer, but make the primary action open the
  // existing Home-Screen installation guide instead of silently disappearing.
  if(iosNeedsInstall){
    if(el.notificationOfferTitle)el.notificationOfferTitle.textContent="לקבל Push מכותרת פלוס באייפון?";
    ...
    localStorage.setItem("hadashota.notificationPromptLastShownAt",String(Date.now()));
...
function syncInstallControl() {
  ...
  const mobile = isIOSDevice() || isAndroidDevice() || /mobile/i.test(navigator.userAgent);
  el.installAppBtn.hidden = !mobile;
  if (!mobile) return;
  const installed = isStandaloneMode() || localStorage.getItem("hadashota.appInstalled") === "1";
  el.installAppBtn.disabled = installed;
  el.installAppBtn.textContent = installed ? "כותרת פלוס מותקנת במכשיר ✓" : "הוספת כותרת פלוס למסך הבית";
}

function maybeShowInstallOffer(reason = "manual") {
  const mobile = isIOSDevice() || isAndroidDevice() || /mobile/i.test(navigator.userAgent);
  if (!mobile) { syncInstallControl(); return; }
...
function refreshNewsOnForeground(reason = "foreground") {
  ...
  // Foreground lifecycle refreshes reuse the shared Worker snapshot. This keeps
  // resume/pageshow fast without turning every focus change into a full scan of
  // all publishers. The explicit refresh button is still allowed to force.
...
async function loadNews(force = false, fromRetry = false, silent = false) {
  ...
  try {
    ...
    // V197: every newsroom refresh (initial, manual and automatic) uses the
...
async function mergeClustersClientResponsive(items) {
  ...
  for (let itemIndex = 0; itemIndex < sorted.length; itemIndex += 1) {
    ...
    if (!match) {
      ...
      // V138: never borrow an image from another report in the cluster. The card
      // links to the representative article, so its image must come from that
      // representative article (or be resolved from that exact page later).
      const imageUrl = preferred.imageUrl || null;
...
function mergeClustersClient(items) {
  ...
  for (const item of sorted) {
    ...
    const latestReportAt = newestIso(reports.map((report) => report.publishedAt).concat([match.latestReportAt, item.latestReportAt]));
    const firstReportAt = oldestIso(reports.map((report) => report.publishedAt).concat([match.firstReportAt, item.firstReportAt]));
    // V138: keep cluster imagery tied to the representative article only.
    const imageUrl = preferred.imageUrl || null;
    const preserved = { ...preferred };
...
function normalizeClusterReports(item) {
  const base = {
    ...
    imageLandingUrl: item.imageLandingUrl || "",
    title: item.title || "",
...
// V230 — legal wording guard (presentation only).
...
// V138 — stable original-article imagery. Successful resolutions are remembered
// across the 30-second feed rerenders; transient failures expire quickly so one
// blocked/slow request cannot hide an existing article image for the whole session.
const ORIGINAL_ARTICLE_IMAGE_CACHE = new Map();
const FEED_IMAGE_MEMORY = new Map();
const BAD_FEED_IMAGE_URLS = new Map();
const ARTICLE_IMAGE_SUCCESS_TTL_MS = 6 * 60 * 60 * 1000;
const ARTICLE_IMAGE_FAILURE_TTL_MS = 45 * 1000;
const FEED_IMAGE_MEMORY_TTL_MS = 6 * 60 * 60 * 1000;
const BAD_FEED_IMAGE_TTL_MS = 10 * 60 * 1000;
const FEED_IMAGE_MEMORY_MAX = 320;

function feedImageMemoryKey(articleUrl) {
...
function pruneFeedImageMemory() {
  const now = Date.now();
  for (const [key, entry] of FEED_IMAGE_MEMORY) {
    if (!entry || now - Number(entry.savedAt || 0) > FEED_IMAGE_MEMORY_TTL_MS) FEED_IMAGE_MEMORY.delete(key);
  }
  while (FEED_IMAGE_MEMORY.size > FEED_IMAGE_MEMORY_MAX) {
    FEED_IMAGE_MEMORY.delete(FEED_IMAGE_MEMORY.keys().next().value);
  }
  for (const [url, until] of BAD_FEED_IMAGE_URLS) {
    if (Number(until || 0) <= now) BAD_FEED_IMAGE_URLS.delete(url);
  }
}
...
function rememberFeedImage(articleUrl, image, provider = "original-article") {
  ...
  if (existing?.provider === "original-article" && provider !== "original-article") return;
  FEED_IMAGE_MEMORY.set(key, {
    ...image,
    url: imageUrl,
    provider,
    savedAt: Date.now()
...
function rememberedFeedImage(articleUrl) {
  const key = feedImageMemoryKey(articleUrl);
  if (!key) return null;
  const entry = FEED_IMAGE_MEMORY.get(key);
  if (!entry) return null;
  if (Date.now() - Number(entry.savedAt || 0) > FEED_IMAGE_MEMORY_TTL_MS || isKnownBadFeedImage(entry.url)) {
    FEED_IMAGE_MEMORY.delete(key);
    return null;
  }
  return entry;
}

function markBadFeedImage(url) {
  const clean = String(url || "").trim();
  if (!clean) return;
  BAD_FEED_IMAGE_URLS.set(clean, Date.now() + BAD_FEED_IMAGE_TTL_MS);
  for (const [key, entry] of FEED_IMAGE_MEMORY) {
    if (entry?.url === clean) FEED_IMAGE_MEMORY.delete(key);
  }
}

function isKnownBadFeedImage(url) {
  const clean = String(url || "").trim();
  if (!clean) return false;
  const until = Number(BAD_FEED_IMAGE_URLS.get(clean) || 0);
  if (!until) return false;
  if (until <= Date.now()) { BAD_FEED_IMAGE_URLS.delete(clean); return false; }
  return true;
}
...
function formatClientPhotoSourceCredit(value, sourceName = "") {
  ...
  if (!raw) return source ? `מקור תמונה: ${source}` : "";
  const prefix = clientPhotoCreditIsAgency(raw) ? "קרדיט תמונה" : "צילום";
  return `${prefix}: ${raw}${source ? ` · מקור: ${source}` : ""}`;
}

function sameImageAssetUrl(a, b) {
  try {
    const A = new URL(String(a || ""), location.origin);
    const B = new URL(String(b || ""), location.origin);
...
function originalClusterSourceImage(item) {
  // Main-story only helper: if the representative image cannot be used, prefer
  // another eligible real image from the SAME event cluster. We never fall back
  // to generic/open-search imagery. Official reports get
...
async function hydrateLeadSafeMedia(winner, leadTitle, publicSnapshot=null) {
  if (!el.leadStoryImage || !el.leadStoryMedia) return;
...
async function hydrateSafeMediaSlot(slot) {
  ...
  const createFallbackSlot = (extraClass = "") => {
    ...
    replacement.className = `safe-media-slot${extraClass ? ` ${extraClass}` : ""}`;
...
function captureVisibleFeedImages() {
  document.querySelectorAll('#feed a.news-image[data-source-url] img[src]').forEach((img) => {
    const a = img.closest('a.news-image');
    if (!a) return;
    const articleUrl = a.dataset.sourceUrl || "";
...
function projectItemForKind(item, kind) {
  ...
  return {
    ...
    imageCredit: representative.imageCredit || "",
    imageCreator: representative.imageCreator || "",
    imageLicense: representative.imageLicense || "",
    imageLicenseUrl: representative.imageLicenseUrl || "",
    imageLandingUrl: representative.imageLandingUrl || "",
...
const hotStorySyncState={lead:{key:"",at:0},display:{key:"",at:0}};
function syncDisplayedHotStoryToServer(story,{displayOnly=false,force=false}={}) {
  if(!story?.fingerprint||!story?.title)return;
  const now=Date.now(),channel=displayOnly?"display":"lead";
  const key=`${story.fingerprint}|${story.title}|${story.sources}|${story.link||""}|${story.image||""}`;
  const ttl=displayOnly?120000:45000,previous=hotStorySyncState[channel];
  if(!force&&key===previous.key&&now-previous.at<ttl)return;
  hotStorySyncState[channel]={key,at:now};
  fetch("/api/hot-story",{method:"POST",headers:{"Content-Type":"application/json","Accept":"application/json"},cache:"no-store",keepalive:true,body:JSON.stringify({...story,displayOnly})}).catch(()=>{});
}
function syncLeadVisualSnapshot(base,imageUrl="") {
  if(!base?.fingerprint||!base?.title)return;
  const visual={...base,image:String(imageUrl||"").trim(),savedAt:Date.now()};
  try{localStorage.setItem("hadashota.publicLead.v1",JSON.stringify(visual));}catch{}
...
function renderLeadStory() {
  ...
  // V217 display policy — the visible hero may be fresher than the strict
  ...
  el.leadStoryImage.removeAttribute("src");
  el.leadStoryMedia.classList.add("image-unavailable");
  hydrateLeadSafeMedia(winner, leadTitle, publicLeadSnapshot);
...
function renderBreaking() {
  // The legacy breaking banner is not present in every newsroom layout.
  // Missing optional markup must not throw on every 30-second render cycle.
  if (!el.breakingBanner || !el.breakingTitle || !el.breakingMeta || !el.breakingLink) return;
  const now = Date.now();
...
function reconcileNotificationPermission() {
  // On iOS a normal Safari tab intentionally has no Notification API. Do not
  // erase a previously stored preference there; the Home-Screen app owns the
...
async function registerServiceWorker() {
  if (!("serviceWorker" in navigator)) return;
  try {
    state.serviceWorkerRegistration = await navigator.serviceWorker.register("/sw.js?v=236.0.0", { updateViaCache: "none" });
...
function homepageInstructionsMarkup() {
  const url="https://koteretplus.com/";
  if (isIOSDevice()) {
...
async function getReadyPushServiceWorkerRegistration() {
  ...
  if (!registration) {
    registration = await navigator.serviceWorker.register("/sw.js?v=236.0.0", { updateViaCache: "none" });
    state.serviceWorkerRegistration = registration;
  }

  // A first-time visitor can tap “enable” while the Service Worker is still
  // installing. Wait for an active registration so PushManager.subscribe does
  // not race the installation lifecycle.
  const readyRegistration = await new Promise((resolve, reject) => {
    const timer = window.setTimeout(() => reject(new Error("service_worker_ready_timeout")), 8000);
  ...
  state.serviceWorkerRegistration = readyRegistration || registration;
  syncPushDeviceIdToServiceWorker(state.serviceWorkerRegistration);
...
async function ensureServerPushSubscription() {
  ...
  const registration = await getReadyPushServiceWorkerRegistration();
...
async function syncPushPreferencesToServer() {
  if (!state.notificationsEnabled || !("Notification" in window) || Notification.permission !== "granted") return false;
  try { await ensureServerPushSubscription(); return true; }
  catch (error) { console.warn("Push preference sync failed", error); return false; }
}
...
function showPushPermissionBlockedHelp(trigger = null) {
  if (!el.notificationOfferModal) {
    showToast("ההתראות חסומות בהגדרות האתר בדפדפן");
    return;
  }
  notificationOfferShownThisSession = true;
  if (el.notificationOfferTitle) el.notificationOfferTitle.textContent = "הדפדפן חסם את ההתראות";
  if (el.notificationOfferText) el.notificationOfferText.innerHTML = "כדי לפתוח מחדש: לחצו על <strong>סמל המנעול / הגדרות האתר</strong> ליד כתובת האתר → <strong>התראות</strong> → <strong>אפשר</strong>. לאחר מכן חזרו לכאן ולחצו ‘נסו שוב’.";
  if (el.notificationOfferAccept) {
    delete el.notificationOfferAccept.dataset.stage;
    el.notificationOfferAccept.disabled = false;
    el.notificationOfferAccept.textContent = "נסו שוב";
  }
  openSiteModal(el.notificationOfferModal, trigger);
}

async function toggleHeadlineNotifications(desiredState = !state.notificationsEnabled, options = {}) {
  const enabled = Boolean(desiredState);
  const quiet = Boolean(options?.quiet);
  if (!enabled) {
    state.notificationsEnabled = false;
    localStorage.setItem("hadashota.headlineNotifications", "0");
    // Reflect the click immediately; network unsubscribe can finish afterwards.
    syncControlsFromState();
    await removeServerPushSubscription();
    if (!quiet) showToast("התראות Push כבויות");
  ...
  if (isIOSDevice() && !isStandaloneMode()) {
    ...
    localStorage.setItem("hadashota.headlineNotifications", "0");
    syncControlsFromState();
    if (!quiet) showToast("באייפון: הוסיפו קודם את כותרת פלוס למסך הבית ואז הפעילו התראות");
    return { ok:false, reason:"ios-install", message:"באייפון יש להוסיף את כותרת פלוס למסך הבית, לפתוח מהאייקון ואז להפעיל התראות." };
  }

  if (!("Notification" in window) || !("serviceWorker" in navigator) || !("PushManager" in window)) {
    if (!quiet) showToast("הדפדפן לא תומך ב-Push");
    return { ok:false, reason:"unsupported", message:"הדפדפן הזה אינו תומך ב‑Web Push." };
  }
  ...
  if (permission !== "granted") {
    ...
    syncControlsFromState();
    if (!quiet) {
      if (permission === "denied") showPushPermissionBlockedHelp();
      else showToast("לא ניתנה הרשאה להתראות");
    }
  ...
  try {
    ...
    localStorage.setItem("hadashota.headlineNotifications", "0");
    syncControlsFromState();
    if (!quiet) showToast("לא הצלחנו לחבר Push ברקע — נסו שוב בעוד רגע");
    return { ok:false, reason:"subscribe", message:"לא הצלחנו לחבר את שירות ה‑Push. נסו שוב בעוד רגע." };
  }
...
function syncControlsFromState() {
  ...
  if (el.pushOrefToggle) { el.pushOrefToggle.checked = state.alertDesktop; el.pushOrefToggle.disabled = !state.notificationsEnabled && !state.alertDesktop; }
...
async function runScheduledRefresh() {
  ...
  // A hidden tab does not need six news API calls every 30 seconds. The
  // visibility/focus handlers perform an immediate shared-snapshot catch-up.
  if (document.hidden) {
    scheduleNextRefresh(60);
    return;
  }
  if (document.querySelector(".site-modal:not(.hidden)")) {
    scheduleNextRefresh(5);
    return;
  }
...
const adViewEventsSent = new Set();
function trackAdEvent(slot, promo, type="view") {
  const id=String(promo?.id||"");
  if(!id)return;
  const key=`${slot}:${id}:${type}`;
  if(type==="view"&&adViewEventsSent.has(key))return;
  if(type==="view")adViewEventsSent.add(key);
...
function feedPromoCardHtml(promo) {
  ...
  const image = promo.imageData
    ? `<img class="feed-promo-image" src="${escapeHtml(promo.imageData)}" alt="${escapeHtml(`פרסום בכותרת פלוס: ${String(promo.text || "").slice(0,90)}`)}" loading="lazy" decoding="async">`
    : "";
  return `<article class="news-card feed-promo-card" aria-label="פרסום">
    <a class="feed-promo-link" href="${escapeHtml(promo.url)}" target="_blank" rel="noopener noreferrer sponsored" data-ad-slot="feed" data-ad-id="${escapeHtml(promo.id||"")}">
      ${image}
      <div class="feed-promo-copy">
...
async function initPromoCard() {
  ...
  const apply = (promo) => {
    ...
    trackAdEvent("top",promo,"view");
    link.onclick = () => trackAdEvent("top",promo,"click");
    link.href = promo.url;
    link.target = "_blank";
    ...
    if (promo.imageData) {
      ...
      image.alt = `פרסום בכותרת פלוס: ${String(promo.text || "").slice(0, 90)}`;
...
async function setAlertDesktop(enabled) {
  ...
  if (state.alertDesktop && ("Notification" in window) && Notification.permission === "granted") {
    ...
    localStorage.setItem("hadashota.notificationPromptChoice", "accepted");
    localStorage.removeItem("hadashota.notificationSnoozeUntil");
    await ensureServerPushSubscription().catch((error)=>console.warn("OREF push subscription sync failed",error));
  } else {
    await syncPushPreferencesToServer().catch(()=>{});
  }
}
...
function koteretFindPreferencesPanel() {
  const heading = [...document.querySelectorAll("h1,h2,h3,strong")].find((el) =>
    /(?:העדפות תצוגה|הגדרות והתראות)/.test(el.textContent || "")
  );
...
function koteretCloseSettings() {
  ...
  const delay = window.matchMedia?.("(max-width: 760px)")?.matches ? 0 : 90;
  setTimeout(() => { modal.hidden = true; }, delay);
}
