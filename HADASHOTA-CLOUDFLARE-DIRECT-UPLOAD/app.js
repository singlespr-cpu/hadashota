const KOTERET_CLIENT_BUILD = "233.0.0";
...
async function verifyApiVersion() {
  ...
  try {
    ...
    marker.textContent = apiVersion ? `גרסה V233 · API ${apiVersion}` : "גרסה V233 · API לא מזוהה";
  } catch (error) {
    marker.textContent = "גרסה V233 · API לא מחובר";
...
async function registerServiceWorker() {
  if (!("serviceWorker" in navigator)) return;
  try {
    state.serviceWorkerRegistration = await navigator.serviceWorker.register("/sw.js?v=233.0.0", { updateViaCache: "none" });
...
async function getReadyPushServiceWorkerRegistration() {
  ...
  if (!registration) {
    registration = await navigator.serviceWorker.register("/sw.js?v=233.0.0", { updateViaCache: "none" });
