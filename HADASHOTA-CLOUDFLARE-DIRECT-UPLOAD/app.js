const KOTERET_CLIENT_BUILD = "153.0.0";
...
async function verifyApiVersion() {
  ...
  try {
    ...
    marker.textContent = apiVersion ? `גרסה V153 · API ${apiVersion}` : "גרסה V153 · API לא מזוהה";
  } catch (error) {
    marker.textContent = "גרסה V153 · API לא מחובר";
...
function setModalBackgroundInert() { /* V153: modal backdrop + focus trap avoid costly body-wide inert writes. */ }
...
async function registerServiceWorker() {
  if (!("serviceWorker" in navigator)) return;
  try {
    state.serviceWorkerRegistration = await navigator.serviceWorker.register("/sw.js?v=153.0.0", { updateViaCache: "none" });
...
async function getReadyPushServiceWorkerRegistration() {
  ...
  if (!registration) {
    registration = await navigator.serviceWorker.register("/sw.js?v=153.0.0", { updateViaCache: "none" });
