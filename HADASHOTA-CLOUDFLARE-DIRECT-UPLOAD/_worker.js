...
const PROMO_ADMIN_PASSWORD_HASH = "b029547edde8d1922c9e07f1b8da5d45423e7e6996dea977d9fb03574411305b";
const ADMIN_SESSION_COOKIE = "kp_admin_session";
const ADMIN_SESSION_MAX_AGE = 60 * 60 * 24 * 365 * 5;
...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/api/media") {
      // V248: illustrative/open-media lookup remains retired. The public UI may show only
    ...
    if (url.pathname === "/api/health") {
      ...
      if (deep) {
        ...
        return json({
          ...
          activeRightsClearedSources: SOURCES.filter(sourceCollectionAllowed).length,
          blockedPermissionRequiredSources: SOURCES.filter((s)=>!sourceCollectionAllowed(s)).length,
...
async function verifyPromoAdminPassword(password){return await sha256Hex(String(password||""))===PROMO_ADMIN_PASSWORD_HASH}
function adminSessionCookieValue(request){
  const raw=String(request.headers.get("Cookie")||"");
  for(const part of raw.split(";")){const i=part.indexOf("=");if(i<0)continue;if(part.slice(0,i).trim()===ADMIN_SESSION_COOKIE)return decodeURIComponent(part.slice(i+1).trim())}
  return "";
}
function adminSessionCookie(token,maxAge=ADMIN_SESSION_MAX_AGE){return `${ADMIN_SESSION_COOKIE}=${encodeURIComponent(String(token||""))}; Max-Age=${Math.max(0,Number(maxAge)||0)}; Path=/api/admin; Secure; HttpOnly; SameSite=Strict`}
async function verifyAdminSession(request,env){
  const token=adminSessionCookieValue(request);if(!token)return false;
  const tokenHash=await sha256Base64Url(token);
  try{const result=await adminHubJson(env,"/admin/session/verify",{tokenHash});return !!result?.ok}catch{return false}
}
async function handleAdminSession(request,env){
  ...
  if(request.method==="GET"){
    ...
    return ok?json({ok:true,persistent:true},200,{"Cache-Control":"no-store"}):json({error:"נדרשת התחברות"},401,{"Cache-Control":"no-store"});
  }
  const token=adminSessionCookieValue(request);
  if(token){const tokenHash=await sha256Base64Url(token);await adminHubJson(env,"/admin/session/delete",{tokenHash}).catch(()=>{})}
  return json({ok:true},200,{"Cache-Control":"no-store","Set-Cookie":adminSessionCookie("",0)});
}
...
async function handleHotStorySync(request,env){
  ...
  const incomingSafeText=String(body?.textPolicy||"")==="facts-only-independent-wording-v248";
  ...
  if(!fingerprint||title.length<6||sources<1||(sources<2&&!(displayOnly&&official)))return json({error:"Invalid hot story"},400,{"Cache-Control":"no-store"});
  ...
  const payload={fingerprint,title,sources,at,firstAt,official,link:cleanHttp(body?.link),image:"",generatedAt:new Date().toISOString(),origin:"site",textPolicy:"facts-only-independent-wording-v248"};
  const stub=pushHubStub(env);
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  <url><loc>${safeOrigin}/terms</loc><changefreq>yearly</changefreq><priority>0.4</priority></url>
</urlset>`;
...
// V248 cost/rights guardrail: legacy article-image scraping and Commons/Openverse
...
function getShardSources(shard) {
  ...
  const shardMap = {
    ...
    "sites-2": sites2,
    "sites-3": sites3,
    "telegram-1": telegram1,
    "telegram-2": telegram2,
    "telegram-3": telegram3,
...
function summarizeSourceHealth(settled, now, cutoff) {
  return settled.map((result) => {
    ...
    return {
      ...
      rightsPolicy: sourceRightsPolicy(result.source).collection,
      factsOnly: sourceRightsPolicy(result.source).factsOnly === true,
      mediaReuseAllowed: sourceRightsPolicy(result.source).media === true,
...
const PUBLIC_NEWS_GENERIC_LABEL = Object.freeze({
  ...
  diplomatic:"התפתחות מדינית חדשה",
  other:"עדכון חדשותי חדש"
});
...
function publicStripPublisherStyle(value){
  return cleanText(String(value||""))
    .replace(/^(?:פרסום ראשון|בלעדי|חשיפה|דרמה|סערה|צפו|תיעוד|תיעוד דרמטי|מתעדכן|דיווח ראשוני|ראשוני|מבזק)\s*[:\-–—]\s*/iu,"")
    .replace(/^(?:לפי דיווחים?|על פי דיווחים?|דיווח:)\s*/iu,"")
    .replace(/\s*\|\s*[^|]{2,30}$/u,"")
...
// V248 — Editorial copyright QA. This is deliberately local/deterministic:
...
function publicHeadlineReadability(value){
  ...
  const awkward=[
    /התקבלה אצל .{2,70} החלטה ל/u,
    /החל(?:ה|ו)? בביצוע את/u,
    /^ל.{2,90} (?:נבחר|נבחרה|מונה|מונתה) .{2,70}$/u,
    /(?:^|\s)בידי (?:של|את|על|עם)(?:\s|$)/u,
    /—\s*(?:של|את|על|עם|אל|כי)(?:\s|$)/u,
    /(?:דווח על|פורסם) (?:כי )?דווח/u
  ];
  if(awkward.some((rx)=>rx.test(t)))return {ok:false,score:0,reason:"awkward-order"};
...
function publicHeadlinePreservesFacts(sourceTitle,rewrittenTitle){
  ...
  const hardGuards=[
    ...
    [/לכאורה/u,/לכאורה/u],
    [/צפוי|עשוי|ייתכן|אפשרי|כנראה/u,/צפוי|עשוי|ייתכן|אפשרי|כנראה/u],
    [/דיווח(?:ים)? על|לפי דיווח/u,/דיווח|לפי/u]
  ];
  for(const [srcRx,outRx] of hardGuards)if(srcRx.test(source)&&!outRx.test(out))return false;
  return true;
}
...
function publicRewriteFactClause(value){
  ...
  if(!fact)return "";
  let m;
  // Numbers / casualties / arrests: preserve the exact number while changing syntax.
  m=fact.match(/^(\d+)\s+בני אדם\s+(נפצעו|נהרגו|נעצרו|פונו)\s+(.+)$/u);
  if(m)return cleanText(`${m[3]} — ${m[2]} ${m[1]} בני אדם`);
  m=fact.match(/^(נפצעו|נהרגו|נעצרו|פונו)\s+(\d+)\s+(?:בני אדם|אנשים)\s+(.+)$/u);
  if(m)return cleanText(`${m[3]} — ${m[1]} ${m[2]} בני אדם`);
  m=fact.match(/^(.{2,56}?)\s+תהיה\s+מקום\s+(\d+)\s+ברשימת\s+(.+)$/u);
  if(m)return cleanText(`${m[1]} תשובץ במקום ${m[2]} ברשימת ${m[3]}`);
  m=fact.match(/^(.{2,56}?)\s+יהיה\s+מקום\s+(\d+)\s+ברשימת\s+(.+)$/u);
  if(m)return cleanText(`${m[1]} ישובץ במקום ${m[2]} ברשימת ${m[3]}`);
  m=fact.match(/^(נעצר|נעצרה)\s+(.{2,100}?)(\s+בחשד(?:\s+ל|\s+ב).+)$/u);
  if(m)return cleanText(`${m[2]} ${m[1]}${m[3]}`);
  m=fact.match(/^(טיל|רקטה|רקטות|טילים|כטב[״"']?ם)\s+(?:שוגר|שוגרה|שוגרו)\s+מ(.+?)\s+לעבר\s+(.+)$/u);
  if(m)return cleanText(`דווח על שיגור ${m[1]} מ${m[2]} לכיוון ${m[3]}`);
  // Common incident syntax.
  m=fact.match(/^שריפה\s+פרצה\s+(.+)$/u);
  if(m)return cleanText(`דווח על שריפה שפרצה ${m[1]}`);
  m=fact.match(/^תאונה\s+אירעה\s+(.+)$/u);
  if(m)return cleanText(`דווח על תאונה שאירעה ${m[1]}`);
  m=fact.match(/^פיצוץ\s+נשמע\s+(.+)$/u);
  if(m)return cleanText(`דווח על פיצוץ שנשמע ${m[1]}`);
  m=fact.match(/^(אזעקות|התרעות)\s+(?:הופעלו|נשמעו)\s+(.+)$/u);
  if(m)return cleanText(`${m[2]} — הופעלו ${m[1]}`);
  ...
  if(m)return cleanText(`דווח על מעצר ${m[3]} בידי ${m[1]}`);
  m=fact.match(/^(.{2,55}?)\s+(אישר|אישרה|אישרו)\s+(.+)$/u);
  if(m){return cleanText(`לפי ${m[1]}, התקבלה החלטה לאשר ${cleanText(m[3])}`);}
  m=fact.match(/^(.{2,55}?)\s+(דרש|דרשה|דרשו|דורש|דורשת|דורשים)\s+(.+)$/u);
  if(m)return cleanText(`${m[1]} הציב דרישה: ${m[3]}`);
  m=fact.match(/^(.{2,55}?)\s+(נפגש|נפגשה|נפגשו)\s+עם\s+(.+)$/u);
  if(m)return cleanText(`נערכה פגישה בין ${m[1]} לבין ${m[3]}`);
  m=fact.match(/^(.+?)\s+(נפצע|נפצעה|נהרג|נהרגה)\s+(.+)$/u);
  if(m)return cleanText(`דווח כי ${m[1]} ${m[2]} ${m[3]}`);
  ...
  // V248 additional high-confidence event grammars. Every rule changes sentence
...
function publicRewriteSingleTitle(title,item={}){
  ...
  if(colon){
    ...
    if(/^הותר לפרסום$/u.test(left)){
      const rewritten=cleanText(`הותר לפרסום כי ${right}`);
      if(publicHeadlinePreservesFacts(base,rewritten)&&publicHeadlineUseful(rewritten))return rewritten;
    }
    const reportVerb=left.match(/^(.{2,52}?)\s+(הכריז|הכריזה|הכריזו|הודיע|הודיעה|הודיעו|אמר|אמרה|אמרו|מסר|מסרה|מסרו|ציין|ציינה|ציינו|טען|טענה|טענו)$/u);
    if(reportVerb){
      const who=cleanText(reportVerb[1]),verb=reportVerb[2];
      const raw=right.replace(/^כי\s+/u,"");
      const fact=publicRewriteFactClause(raw)||raw;
      let rewritten;
      if(/^על\s+/u.test(raw)&&/^(?:הודיע|הודיעה|הודיעו|הכריז|הכריזה|הכריזו)$/u.test(verb))rewritten=cleanText(`פורסם עדכון מטעם ${who} ${raw}`);
      else if(/^(?:טען|טענה|טענו)$/u.test(verb))rewritten=cleanText(`לטענת ${who}, ${fact}`);
      else if(/^(?:אמר|אמרה|אמרו|ציין|ציינה|ציינו|מסר|מסרה|מסרו)$/u.test(verb))rewritten=cleanText(`לדברי ${who}, ${fact}`);
      else rewritten=cleanText(`לפי הודעת ${who}, ${fact}`);
      if(publicHeadlinePreservesFacts(base,rewritten)&&publicHeadlineUseful(rewritten))return rewritten;
    }
    if(/^(?:צה[״"]ל|פיקוד העורף|משטרת ישראל|המשטרה|מד[״"]א|משרד הבריאות|הכנסת|הממשלה|לשכת ראש הממשלה|שב[״"]כ)$/u.test(left)){
      let officialFact="",fm;
      fm=right.match(/^תקפנו\s+(הלילה|הבוקר|הערב|אמש|היום)\s+(.+)$/u); if(fm)officialFact=`${fm[1]} בוצעה תקיפה נגד ${fm[2]}`;
      if(!officialFact){fm=right.match(/^תקפנו\s+(.+)$/u); if(fm)officialFact=`בוצעה תקיפה נגד ${fm[1]}`;}
      ...
      if(!officialFact){fm=right.match(/^חשד\s+ל(.+)$/u);if(fm)officialFact=`קיים חשד ל${fm[1]}`;}
      if(!officialFact){fm=right.match(/^חשד\s+ש(.+)$/u);if(fm)officialFact=`קיים חשד ש${fm[1]}`;}
      const fact=officialFact||publicRewriteFactClause(right)||right;
      const rewritten=cleanText(`לפי ${left}, ${fact}`);
      if(publicHeadlinePreservesFacts(base,rewritten)&&publicHeadlineUseful(rewritten))return rewritten;
    }
    // If the prefix is contextual rather than attribution, try to rewrite the
    // factual clause alone and keep the context as a suffix, not as a verbatim title.
    const changedRight=publicRewriteFactClause(right);
    if(changedRight){
      const rewritten=cleanText(`${changedRight} — ${left}`);
      if(publicHeadlinePreservesFacts(base,rewritten)&&publicHeadlineUseful(rewritten))return rewritten;
    }
    return "";
  }

  const directFact=publicRewriteFactClause(base);
  ...
  for(const [rx,fn] of patterns){
    ...
    const rewritten=cleanText(fn(m));
    if(publicHeadlinePreservesFacts(base,rewritten)&&publicHeadlineUseful(rewritten))return rewritten;
  }
  return "";
}

function publicTitleLooksPlainFactual(value){
  const t=publicStripPublisherStyle(value);
  // V245: do not reject ordinary breaking-news facts merely because they are
  // short. The fallback remains unavailable for quotes, questions, exclamation
  // copy and promotional/expressive language.
  ...
  if(/(?:לא תאמינו|צפו|תיעוד|דרמה|סערה|הלם|חשיפה|בלעדי|כך זה נראה|הסיבה ש|מה עומד מאחורי|התגובה ש|המסר ש|כל מה שצריך לדעת|הסיפור שמסעיר|הפרטים שלא ידעתם|מיוחד|מטורף|מדהים)/iu.test(t))return false;
  // Require at least a factual anchor: an action/state, a number, a concrete
  // time/place relation, or a known institutional/political actor.
...
function publicConservativeRestatement(title,item={}){
  ...
  const rules=[
    ...
    [/^(.{2,55}?)\s+(פתח|פתחה|פתחו)\s+בחקירת\s+(.+)$/u,(x)=>`נפתחה חקירה של ${x[3]} בידי ${x[1]}`],
  ...
  if(m){
    const changed=publicRewriteFactClause(m[2]);
    if(changed){
      const rewritten=cleanText(`${changed} — ${m[1]}`);
      const qa=publicHeadlineQa(base,rewritten);
      if(qa.ok)return rewritten;
    }
  }
  m=base.match(/^(.{8,72}?)\s+(?:–|—|;)\s+(.{10,})$/u);
  if(m){
    const changed=publicRewriteFactClause(m[2]);
    if(changed){
      const rewritten=cleanText(`${changed} — ${m[1]}`);
  ...
  // V248 broad factual transforms. These change sentence structure rather than
  ...
  const broadRules=[
    ...
    [/^(.{2,58}?)\s+(החליט|החליטה|החליטו)\s+ל(.+)$/u,(x)=>`${x[1]} קיבל החלטה ל${x[3]}`],
    ...
    [/^(.{2,58}?)\s+(מסר|מסרה|מסרו)\s+כי\s+(.+)$/u,(x)=>`לדברי ${x[1]}, ${x[3]}`]
  ];
  const naturalRules=[
    [/^(.{2,58}?)\s+(זכה|זכתה|זכו)\s+ב(.+)$/u,(x)=>`דווח על זכייה של ${x[1]} ב${x[3]}`],
    [/^(.{2,58}?)\s+(ניצח|ניצחה|ניצחו)\s+את\s+(.+)$/u,(x)=>`דווח על ניצחון של ${x[1]} על ${x[3]}`],
    [/^(.{2,58}?)\s+(הפסיד|הפסידה|הפסידו)\s+ל(.+)$/u,(x)=>`דווח על הפסד של ${x[1]} ל${x[3]}`],
    [/^(.{2,58}?)\s+(השיק|השיקה|השיקו)\s+את\s+(.+)$/u,(x)=>`דווח על השקת ${x[3]} בידי ${x[1]}`],
    [/^(.{2,58}?)\s+(פתח|פתחה|פתחו)\s+בחקירה\s+(?:נגד|בעניין)\s+(.+)$/u,(x)=>`נפתחה חקירה בעניין ${x[3]} בידי ${x[1]}`],
    [/^(.{2,58}?)\s+(הגיש|הגישה|הגישו)\s+כתב אישום\s+נגד\s+(.+)$/u,(x)=>`כתב אישום נגד ${x[3]} הוגש בידי ${x[1]}`]
  ];
  for(const [rx,fn] of naturalRules){
    const hit=base.match(rx);if(!hit)continue;
    const rewritten=cleanText(fn(hit));
    const qa=publicHeadlineQa(base,rewritten);
    if(qa.ok)return rewritten;
...
function serverPublicHeadlineResult(item={}){
  ...
  if(candidates.length){
    ...
    candidates.sort((a,b)=>Number(b.support||0)-Number(a.support||0) || Number(b.readability||0)-Number(a.readability||0) || b.anchorCoverage-a.anchorCoverage || a.similarity-b.similarity || detail(b.title)-detail(a.title) || b.title.length-a.title.length);
...
function serverPublicHeadline(item={}){
  return serverPublicHeadlineResult(item).title;
}

function publicReusableSourceImage(row={}){
  const url=String(row?.imageUrl||row?.image||"").trim();
...
function publicNewsReport(row={}, fallbackCategory="other"){
  const base={...row,category:row?.category||fallbackCategory,related:[]};
  const headline=serverPublicHeadlineResult(base),title=headline.title;
  const media=publicReusableSourceImage(row)||{};
  return {
    id:String(row?.id||"").slice(0,220),sourceId:String(row?.sourceId||"").slice(0,120),publisher:String(row?.publisher||"").slice(0,120),
    ...
    attributionLabel:`נוסח כותרת פלוס · מקור הדיווח: ${cleanText(row?.sourceName||row?.publisher||"מקור חיצוני")}`,textPolicy:"facts-only-independent-wording-v248"
  };
}
...
function publicNewsItem(item={}){
  ...
  return {
    ...
    verified:item?.verified===true,official:item?.official===true,independent:item?.independent===true,url:String(item?.url||"").slice(0,1800),publishedAt:String(item?.publishedAt||"").slice(0,64),
    ...
    attributionLabel:`נוסח כותרת פלוס · מקור הדיווח: ${cleanText(item?.sourceName||item?.publisher||"מקור חיצוני")}`,textPolicy:"facts-only-independent-wording-v248"
  };
}
...
async function handleNews(request, env, ctx) {
  ...
  try {
    ...
    // it through /api/news or /api/news-bundle.
    const internalCurrent = jsonText(rawPayloadText, 200, { "Cache-Control": "public, max-age=0, s-maxage=7200" });

    loggedWaitUntil(ctx, Promise.all([
      cache.put(cacheKey, sharedSnapshotResponse),
      cache.put(lastGoodKey, lastGoodResponse),
      cache.put(internalNewsCacheKey(shard,false), internalCurrent)
...
async function fetchSource(source, retryBudget = { remaining: 0 }, forceFresh = false, deadlineAt = Infinity) {
  ...
  while (attempt < 2) {
    try {
      ...
      // V226: preserve explicit photographer/photo-credit metadata whenever it
...
async function readEscalationNewsCache(request){
  ...
  for(const shard of ESCALATION_SHARDS){
    const hit=await cache.match(internalNewsCacheKey(shard,false));if(!hit)continue;
...
// V217 — display remains permissive, while a guarded bridge can promote a fresh multi-source hero to Push.
// The strict selector still defines high-confidence/Breaking Push. This display
// selector is more permissive during quiet hours; V217 may later bridge only a
// fresh multi-source display winner into the autonomous Push path.
const SERVER_DISPLAY_LEAD_MAX_AGE_MS=8*60*60*1000;
function selectServerDisplayLead(items,now=Date.now(),pushWinner=null){
  // A fresh strict Push-qualified winner remains authoritative. Once it is older
  // than an hour, however, a genuinely fresher display candidate may take the
  // hero; V217 can later bridge that NEW hero to Push when it has 2+ sources.
  if(pushWinner&&Number(pushWinner.ageMinutes)<=60){pushWinner.displayReason="push-qualified-fresh";return pushWinner;}
  const corroborationWindowMs=150*60*1000;
  const entries=(items||[]).map((item)=>{
    const latestAt=serverClusterLatestAt(item);
    const latestMs=Date.parse(latestAt);
    ...
    const hotScore=serverStoryHotScore(item,now);
    const editorial=serverLeadEditorialMetrics({uniqueSources,ageMinutes,spreadMinutes,hotScore,hasOfficial,hasVerified});
    return {item,reports,recentReports,uniqueSources,ageMinutes,latestAt,score:editorial.score,hotScore,hasOfficial,hasVerified,spreadMinutes,breaking:editorial.breaking,velocityBoost:editorial.velocityBoost,burstBoost:editorial.burstBoost,agePenalty:editorial.agePenalty};
  });
  const sort=(a,b)=>b.score-a.score||Date.parse(b.latestAt||0)-Date.parse(a.latestAt||0)||String(a.item?.id||a.item?.url||a.item?.title||"").localeCompare(String(b.item?.id||b.item?.url||b.item?.title||""),"he");

  // Quiet-hour display policy:
  // 1) Prefer a genuinely fresh story corroborated by 2+ publishers.
  // 2) A single OFFICIAL source may lead briefly when it is fresh enough.
  // 3) If there is no fresh option, widen the display window gradually instead
  //    of falling back to a waiting placeholder. Only the guarded V217 bridge
  //    may promote a NEW multi-source display winner to Push.
  let reason="";
  let pool=entries.filter((e)=>(e.uniqueSources>=2&&e.ageMinutes<=75)||(e.hasOfficial&&e.uniqueSources>=1&&e.ageMinutes<=45&&e.hotScore>=30)).sort(sort);
  let winner=pool[0]||null;
  if(winner)reason=winner.uniqueSources>=2?"quiet-fresh-2plus":"quiet-fresh-official";
  if(!winner){
    pool=entries.filter((e)=>(e.uniqueSources>=2&&e.ageMinutes<=180)||(e.hasOfficial&&e.uniqueSources>=1&&e.ageMinutes<=120)).sort(sort);
    winner=pool[0]||null;
    if(winner)reason="quiet-expanded";
  }
  if(!winner){
    pool=entries.filter((e)=>(e.uniqueSources>=2&&e.ageMinutes<=480)||(e.hasOfficial&&e.uniqueSources>=1&&e.ageMinutes<=360)).sort(sort);
    winner=pool[0]||null;
    if(winner)reason="quiet-reserve";
  }
  if(!winner)return null;
  winner.fingerprint=serverLeadFingerprint(winner);
  winner.displayReason=reason;
  return winner;
}

function serverLeadDisplaySummary(entry,now=Date.now(),pushQualified=false){
  const base=serverLeadPayload(entry);
  if(!base)return null;
  return {...base,receivedAt:new Date(now).toISOString(),displayOnly:true,displayReason:String(entry?.displayReason||"display"),pushQualified:pushQualified===true};
}

// V217 — the public hero is an editorial decision, not just decoration. If a
// genuinely NEW hero is fresh and corroborated by multiple independent sources,
// it may become the autonomous Push candidate even when it sits just below the
// older strict selector. This keeps headline + Push aligned without turning
// quiet-hour single-source fallbacks into notifications. After 60 minutes with
// no Push, the bridge widens modestly so a credible new multi-source lead does
// not leave subscribers silent for hours. It NEVER repeats the same story simply
// because an hour passed.
function serverDisplayLeadPushBridge(entry,lastPushedState,now=Date.now()){
  if(!entry?.item||!entry?.fingerprint)return {eligible:false,reason:"no-display-lead",minutesSinceLastPush:null};
  const sources=Math.max(0,Number(entry.uniqueSources)||0);
  const age=Math.max(0,Number(entry.ageMinutes)||0);
  const score=Math.max(0,Number(entry.score)||0);
  const hot=Math.max(0,Number(entry.hotScore)||0);
  const reason=String(entry.displayReason||"");
  const lastPushedAt=Date.parse(lastPushedState?.pushedAt||0);
  const minutesSinceLastPush=Number.isFinite(lastPushedAt)?Math.max(0,(now-lastPushedAt)/60000):Infinity;
  const probe=serverLeadPayload(entry);
  if(lastPushedState?.fingerprint&&probe&&leadPushSameStory(lastPushedState,probe)){
    return {eligible:false,reason:"same-last-pushed-story",minutesSinceLastPush};
  }
  // A single official report may be useful enough to show during a quiet hour,
  // but the user explicitly wants Push to remain multi-source by default.
  if(sources<2)return {eligible:false,reason:"single-source-display",minutesSinceLastPush};
  if(age>180)return {eligible:false,reason:"display-too-old-for-push",minutesSinceLastPush};

  // Fresh multi-source hero: the newsroom has already promoted it above the
  // stream, so let that editorial promotion drive Push as well. No cooldown is
  // imposed — a real new event may replace another headline after only minutes.
  if(age<=75&&(reason==="quiet-fresh-2plus"||sources>=3)){
    return {eligible:true,reason:"fresh-main-headline",minutesSinceLastPush};
  }

  // One-hour safety net. This is NOT an hourly timer: it only applies to a NEW
  // different hero with 2+ corroborating sources. A modest quality floor keeps
  // weak reserve stories from being promoted merely because traffic is quiet.
  if(minutesSinceLastPush>=60&&age<=150&&(reason==="quiet-expanded"||reason==="quiet-fresh-2plus")&&(score>=52||hot>=28||entry.hasOfficial===true||entry.hasVerified===true)){
    return {eligible:true,reason:"hourly-credible-main-headline",minutesSinceLastPush};
  }
  return {eligible:false,reason:"display-below-push-bridge",minutesSinceLastPush};
}


// V217 — tiered quiet-hour Push safety net. This does not create an hourly
// timer and never repeats the last pushed story. It only widens the acceptable
// window after prolonged silence when a DIFFERENT story has real multi-source
// corroboration. Breaking/fresh heroes still use the faster V217 path above.
function serverPushSafetyEntry(items,lastPushedState,now=Date.now()){
  const lastPushedAt=Date.parse(lastPushedState?.pushedAt||0);
  const minutesSinceLastPush=Number.isFinite(lastPushedAt)?Math.max(0,(now-lastPushedAt)/60000):Infinity;
  if(minutesSinceLastPush<60)return {entry:null,reason:"under-60-minutes",minutesSinceLastPush};
  const corroborationWindowMs=150*60*1000;
  const entries=(items||[]).map((item)=>{
    const latestAt=serverClusterLatestAt(item);
    const latestMs=Date.parse(latestAt);
    ...
    const hotScore=serverStoryHotScore(item,now);
    const editorial=serverLeadEditorialMetrics({uniqueSources,ageMinutes,spreadMinutes,hotScore,hasOfficial,hasVerified});
    const entry={item,reports,recentReports,uniqueSources,ageMinutes,latestAt,score:editorial.score,hotScore,hasOfficial,hasVerified,spreadMinutes,breaking:editorial.breaking,velocityBoost:editorial.velocityBoost,burstBoost:editorial.burstBoost,agePenalty:editorial.agePenalty};
    entry.fingerprint=serverLeadFingerprint(entry);
    return entry;
  }).filter((e)=>e.item&&e.fingerprint&&e.uniqueSources>=2);

  const isSameLast=(entry)=>{
    if(!lastPushedState?.fingerprint)return false;
    const probe=serverLeadPayload(entry);
    return !!probe&&leadPushSameStory(lastPushedState,probe);
  };
  const sort=(a,b)=>{
    // Recency matters more in the safety net than historical source volume.
    const pa=(Number(a.score)||0)+Math.min(18,Number(a.uniqueSources||0)*2)-Math.min(30,Math.max(0,Number(a.ageMinutes||0)-60)*0.12);
    const pb=(Number(b.score)||0)+Math.min(18,Number(b.uniqueSources||0)*2)-Math.min(30,Math.max(0,Number(b.ageMinutes||0)-60)*0.12);
    return pb-pa||Date.parse(b.latestAt||0)-Date.parse(a.latestAt||0);
  };
  const freshDifferent=entries.filter((e)=>!isSameLast(e)).sort(sort);
  let pool=[]; let reason="";

  // 60–120 minutes without a lead Push: require a genuinely recent new story.
  if(minutesSinceLastPush>=60){
    pool=freshDifferent.filter((e)=>e.ageMinutes<=120&&(e.uniqueSources>=3||e.hasOfficial||e.hasVerified||e.score>=28||e.hotScore>=18)).sort(sort);
    if(pool.length)reason="hourly-multisource-safety";
  }
  // 120+ minutes: widen to three hours, still requiring 2+ sources and a
  // modest credibility/importance signal.
  if(!pool.length&&minutesSinceLastPush>=120){
    pool=freshDifferent.filter((e)=>e.ageMinutes<=180&&(e.uniqueSources>=3||e.hasOfficial||e.hasVerified||e.score>=20||e.hotScore>=12)).sort(sort);
    if(pool.length)reason="extended-multisource-safety";
  }
  // 180+ minutes: do not stay silent merely because the night is quiet. A
  // different story may be up to four hours old, but must have stronger support
  // (3+ reports, or 2+ with official/verified backing). Never push a lone item.
  if(!pool.length&&minutesSinceLastPush>=180){
    pool=freshDifferent.filter((e)=>e.ageMinutes<=240&&(e.uniqueSources>=3||(e.uniqueSources>=2&&(e.hasOfficial||e.hasVerified)))).sort(sort);
    if(pool.length)reason="long-silence-credible-story";
  }
  const winner=pool[0]||null;
  if(winner)winner.displayReason=`push-${reason}`;
  return {entry:winner,reason:winner?reason:"no-credible-new-story",minutesSinceLastPush};
}

// V217 diagnostics: show the strongest candidates even when none is selected,
// so /api/push/status explains WHY the newsroom stayed silent.
function serverPushCandidateDiagnostics(items,lastPushedState,selectedEntry,strictEntry,now=Date.now()){
  const lastPushedAt=Date.parse(lastPushedState?.pushedAt||0);
  const minutesSinceLastPush=Number.isFinite(lastPushedAt)?Math.max(0,(now-lastPushedAt)/60000):Infinity;
  const corroborationWindowMs=150*60*1000;
  const entries=(items||[]).map((item)=>{
    ...
    const hotScore=serverStoryHotScore(item,now);
    const editorial=serverLeadEditorialMetrics({uniqueSources,ageMinutes,spreadMinutes,hotScore,hasOfficial,hasVerified});
    const entry={item,reports,recentReports,uniqueSources,ageMinutes,latestAt,score:editorial.score,hotScore,hasOfficial,hasVerified,spreadMinutes,breaking:editorial.breaking};
    entry.fingerprint=serverLeadFingerprint(entry);
    return entry;
  }).filter((e)=>e.item&&e.fingerprint).sort((a,b)=>b.score-a.score||Date.parse(b.latestAt||0)-Date.parse(a.latestAt||0));

  return entries.slice(0,3).map((e,index)=>{
    const probe=serverLeadPayload(e);
    const sameLast=!!lastPushedState?.fingerprint&&!!probe&&leadPushSameStory(lastPushedState,probe);
    const selected=!!selectedEntry&&String(selectedEntry.fingerprint||"")===String(e.fingerprint||"");
    const strict=!!strictEntry&&String(strictEntry.fingerprint||"")===String(e.fingerprint||"");
    let reason="below-policy-threshold";
    if(selected)reason="selected-for-push";
    else if(strict)reason="strict-qualified";
    else if(sameLast)reason="same-last-pushed-story";
    else if(e.uniqueSources<2)reason="single-source";
    else if(e.ageMinutes>240)reason="older-than-4h";
    else if(minutesSinceLastPush<60&&e.ageMinutes>75)reason="waiting-for-60m-safety-window";
    else if(minutesSinceLastPush>=60&&e.ageMinutes<=120)reason="eligible-window-but-quality-lower";
    else if(minutesSinceLastPush>=120&&e.ageMinutes<=180)reason="extended-window-but-quality-lower";
    else if(minutesSinceLastPush>=180&&e.ageMinutes<=240)reason="long-silence-window-but-support-lower";
    return {rank:index+1,title:serverPublicHeadline(e.item||{})||"(לא פורסם: אין ניסוח עצמאי בטוח)",fingerprint:e.fingerprint,sources:e.uniqueSources,ageMinutes:Math.max(0,Math.round(e.ageMinutes)),score:Math.max(0,Math.round(Number(e.score)||0)),hotScore:Math.max(0,Math.round(Number(e.hotScore)||0)),spreadMinutes:Math.max(0,Math.round(Number(e.spreadMinutes)||0)),official:e.hasOfficial===true,verified:e.hasVerified===true,breaking:e.breaking===true,selectedForPush:selected,strictQualified:strict,sameAsLastPushed:sameLast,reason};
  });
}
...
function serverLeadPayload(entry) {
  ...
  const item=entry.item;
  const publicTitle=serverPublicHeadline(item);
  if(!publicHeadlineUseful(publicTitle))return null;
...
function serverLeadDisplaySnapshot(entry){
  ...
  const publicTitle=serverPublicHeadline(item);
  if(!publicHeadlineUseful(publicTitle))return null;
  const cleanReport=(row={})=>publicNewsReport(row,String(row?.category||item?.category||"other"));
  ...
  const displayItem={
    ...
    preview:"",
    imageUrl:null,imageCredit:"",imageCreator:"",imageLicense:"",imageLicenseUrl:"",imageLandingUrl:"",
  ...
  return {
    ...
    uniqueSources:Math.max(1,Math.min(100,Number(entry.uniqueSources)||recent.length||reports.length||1)),
...
function serverRecoverDisplayEntryForLatest(items=[], latest=null, now=Date.now()){
  ...
  for(const item of Array.isArray(items)?items:[]){
    ...
    entry.fingerprint=serverLeadFingerprint(entry);
    const probe={fingerprint:entry.fingerprint,title:serverPublicHeadline(item||{}),firstAt:serverClusterFirstAt(item),at:latestAt};
...
async function readBackgroundShardPayload(shard){
  try{
    const key=internalNewsCacheKey(shard,false);
...
async function storeBackgroundMonitor(storage,input={},existingSnapshot=undefined){
  ...
  const existing=existingSnapshot===undefined?await storage.get("push.backgroundMonitor"):existingSnapshot;
...
async function fetchBackgroundShardIsolatedRaw(env,shard,tickStartedAt=""){
  const stub=pushShardCollectorStub(env,shard);
  if(!stub)throw new Error("PUSH_HUB_NOT_BOUND");
  // V236 CPU guard: keep the every-minute collector payload serialized while
  // it crosses the thin Cron orchestrator. PushHub parses it once where the
  // newsroom/lead logic already runs.
  const response=await backgroundDeadline(
    stub.fetch("https://push.internal/collect-news-shard",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({shard,tickStartedAt,rawPayload:true})}),
    12000,
    `SHARD_DO_TIMEOUT_${shard}`
  );
  if(!response.ok){
  ...
  const generated=Date.parse(response.headers.get("X-KP-Generated-At")||0);
  const stale=response.headers.get("X-KP-Stale")==="1";
  const servedFromCache=response.headers.get("X-KP-Served-From-Cache")==="1";
  if(stale||servedFromCache||!Number.isFinite(generated)||Date.now()-generated>5*60*1000)throw new Error("INVALID_SHARD_DO_PAYLOAD");
  const raw=await response.text();
  if(!raw||raw[0]!=="{"||raw[raw.length-1]!=="}")throw new Error("INVALID_SHARD_DO_PAYLOAD");
  return raw;
}

async function refreshBackgroundNewsShardsRaw(env,ctx,shards=[],tickStartedAt=""){
  const fresh=new Map(),failures={};
  await Promise.all(shards.map(async(shard)=>{
    try{
      const raw=await fetchBackgroundShardIsolatedRaw(env,shard,tickStartedAt);
      fresh.set(shard,raw);
    }catch(error){
      failures[shard]=String(error?.message||error);
      console.warn(`V237 DO background shard ${shard} failed`,failures[shard]);
    }
  }));
...
async function collectServerRecentNewsContext(){
  // V236: escalation consumes recent rows only. This is the exact pre-cluster
  // portion of collectServerPushContext, without computing an unused lead.
  const now=Date.now(),cutoff=now-30*60*60*1000;
  const payloads=[];
  for(const shard of ESCALATION_SHARDS){
    const payload=await readBackgroundShardPayload(shard);
  ...
  return {recent,now,shards:payloads.length};
}
...
async function runBackgroundEscalationFromNews(env,recent=[],orefSnapshot=null) {
  ...
  try{
    ...
    const [oref,idfWeb,nscWeb]=await Promise.all([orefSnapshot?.ok?Promise.resolve(orefSnapshot):fetchOrefForEscalation(),fetchIdfOfficialForEscalation(),fetchNscOfficialForEscalation()]);
...
async function runBackgroundPushMonitor(env, ctx) {
  ...
  try {
    ...
    phase="collectors";
    const fresh=await refreshBackgroundNewsShardsRaw(env,ctx,ESCALATION_SHARDS,tickStartedAt);
    // V237: heartbeat already marks this tick RUNNING, and any later failure is
    // persisted by report("failed"). Avoid a second diagnostics-only DO request
    // after collectors; /background-news records the full shard/failure status.

    // V236: assemble the PushHub request from already serialized collector JSON.
    // This removes six large JSON.parse operations and one large JSON.stringify
    // from the 10ms scheduled Worker, without changing the object PushHub sees.
    const shardJson=[...fresh.entries()].map(([shard,raw])=>`${JSON.stringify(shard)}:${raw}`).join(",");
    const backgroundBody=`{"shards":{${shardJson}},"collectorFailures":${JSON.stringify(fresh.failures||{})},"generatedAt":${JSON.stringify(new Date().toISOString())},"tickStartedAt":${JSON.stringify(tickStartedAt)},"expectedShards":${ESCALATION_SHARDS.length}}`;
    phase="background-news";
    const backgroundResponse=await backgroundDeadline(
      stub.fetch("https://push.internal/background-news",{method:"POST",headers:{"Content-Type":"application/json"},body:backgroundBody}),
      15000,
      "BACKGROUND_NEWS_TIMEOUT"
    );
    if(!backgroundResponse.ok)throw new Error(`BACKGROUND_NEWS_HTTP_${backgroundResponse.status}`);
    // /background-news performs the /lead decision inside PushHub. Its returned
    // diagnostic body is not consumed because nothing in the Cron uses it.

    // V237: /background-news already persists the COMPLETE monitor state inside
    // PushHub after the lead decision. Do not send a second identical DO request
    // from the Cron wrapper. This removes one DO request + monitor read/write per
    // minute while preserving the exact newsroom success signal.
    phase="complete";

    // OREF remains independent. A successful result is reused by the five-minute
    // escalation pass in this SAME Cron invocation; on OREF failure escalation
    // keeps its original independent fallback fetch.
    let orefSnapshot=null;
    try{
      const controller=new AbortController();
      ...
      try{
        ...
        if(r.ok){const raw=(await r.text()).replace(/^\uFEFF/,"").trim(),parsed=raw&&raw!=="null"?JSON.parse(raw):null,alerts=normalizeOrefCurrentAlerts(parsed);orefSnapshot={ok:true,alerts};if(alerts.length)await queueOrefAlerts(env,alerts);}
      }finally{clearTimeout(timer);}
    }catch(error){console.warn("Scheduled OREF push monitor failed",error);}

    if(minute%5===0){
      try{const escalationContext=await collectServerRecentNewsContext();await runBackgroundEscalationFromNews(env,escalationContext.recent,orefSnapshot);}
      catch(error){console.warn("Scheduled escalation monitor failed",error);}
    }
  } catch(error) {
    const message=String(error?.message||error);
    console.warn("V237 scheduled push monitor failed",phase,message);
...
function analyticsSourceHintGroup(value="") {
  const x=String(value||"").trim().toLowerCase();
  if(["google","meta","x","whatsapp","share"].includes(x))return x;
  return "";
}
...
function analyticsTrimCounts(value={},limit=30) {
  return Object.fromEntries(Object.entries(value||{}).filter(([k,v])=>k&&Number(v)>0).sort((a,b)=>Number(b[1])-Number(a[1])).slice(0,limit));
}
function analyticsReferrerGroup(value="",hint="") {
  const sourceHint=analyticsSourceHintGroup(hint);
  if(value){
    try {
...
// V220 — source growth is NOT a material update. A story may gain 5, 10 or 20
// publishers while remaining the very same event; that must never fan out a
// second Push by itself. Only a concrete factual development (changed casualty
// number/severity, death, arrest/capture, neutralisation, official terror-event
// classification, release/rescue, etc.) can unlock another Push for the same
// event. The signature is intentionally based on facts, not wording or sources.
function leadPushMaterialFacts(value=""){
  const text=normalizeHebrew(value||"");
  const tokens=new Set(text.split(/\s+/).filter(Boolean));
  // Count only event-impact numbers (casualties / launches / hostages etc.).
  // Incidental numbers such as an age, road number or time must not create a
  // second Push for the same event.
  const numberFacts=[];
  const countWord="הרוג(?:ים|ות)?|פצוע(?:ים|ות)?|נפגע(?:ים|ות)?|נעדר(?:ים|ות)?|חטופ(?:ים|ות)?|טיל(?:ים)?|רקט(?:ה|ות)|שיגור(?:ים)?";
  for(const match of text.matchAll(new RegExp(`(\\d+(?:[.,]\\d+)?)\\s+(${countWord})|(${countWord})\\s+(\\d+(?:[.,]\\d+)?)`,`g`))){
    const n=match[1]||match[4]||"";const label=match[2]||match[3]||"";if(n&&label)numberFacts.push(`${label}:${n}`);
  }
  if(/(?:פצוע|נפגע|הרוג|נהרג|נרצח|נעדר|חטופ)/.test(text)){
    for(const match of text.matchAll(/(?:עלה|עלתה|עומד|עומדת)\s+ל\s+(\d+(?:[.,]\d+)?)/g))numberFacts.push(`impact:${match[1]}`);
  }
  const numbers=[...new Set(numberFacts)].sort().join("|");
  const families=[];
  const hasAny=(words)=>words.some((w)=>tokens.has(w)||text.includes(w));
  const groups=[
    ["death",["נהרג","נהרגה","נהרגו","נרצח","נרצחה","נרצחו","נפטר","נפטרה","נפטרו","מותו","מותה","מת מפצעיו","מתה מפצעיה","נקבע מותו","נקבע מותה"]],
    ["critical",["אנוש","אנושה","קריטי","קריטית","סכנה לחייו","סכנה לחייה"]],
    ["severe",["קשה","קשים","קשות"]],
    ["moderate",["בינוני","בינונית","בינוניים","בינוניות"]],
    ["arrest",["נעצר","נעצרה","נעצרו","נתפס","נתפסה","נתפסו","נלכד","נלכדה","נלכדו","הסגיר","הסגירה"]],
    ["neutralized",["חוסל","חוסלה","חוסלו","נוטרל","נוטרלה","נוטרלו","נורה למוות"]],
    ["terror-classified",["פיגוע","טרור","לאומני","לאומנית"]],
    ["released",["שוחרר","שוחררה","שוחררו","חולץ","חולצה","חולצו","אותו בחיים","אותרה בחיים"]],
    ["hostage",["חטוף","חטופה","חטופים","חטופות"]],
    ["ceasefire",["הפסקת אש","הפסקת האש","הסכם הפסקת אש"]]
  ];
  for(const [name,words] of groups)if(hasAny(words))families.push(name);
  return {numbers,families:[...new Set(families)].sort()};
}
function leadPushMaterialDevelopment(previousState={},payload={}){
  const titleDelta=leadPushTitleDelta(previousState?.title||"",payload?.title||"");
  const before=leadPushMaterialFacts(previousState?.title||"");
  const after=leadPushMaterialFacts(payload?.title||"");
  const addedFamilies=after.families.filter((x)=>!before.families.includes(x));
  const numbersChanged=!!(before.numbers||after.numbers)&&before.numbers!==after.numbers;
  // A factual update must ADD a concrete development, or change a reported
  // number. Merely removing/rephrasing words never qualifies.
  const material=titleDelta.changed&&(numbersChanged||addedFamilies.length>0);
  const signature=`n:${after.numbers||"-"}|f:${after.families.join(",")||"-"}`;
  return {material,numbersChanged,addedFamilies,signature,titleDelta};
}
function leadPushEventId(payload={},priorState=null){
  if(priorState?.eventId&&leadPushSameStory(priorState,payload))return String(priorState.eventId);
  const first=Date.parse(payload?.firstAt||payload?.at||0);
  const bucket=Number.isFinite(first)?Math.floor(first/(20*60*1000)):0;
  return pushIdentityHash(`${bucket}|${String(payload?.fingerprint||"")}`);
}
...
async function rememberLeadPush(storage,payload,state={}){
  ...
  rows.push({
    ...
    eventId:String(state?.eventId||leadPushEventId(payload,state)||""),
    materialSignature:String(state?.materialSignature||leadPushMaterialFacts(payload?.title||"").families.join(",")||""),
...
function safeStoredNotificationForDelivery(notification){
  ...
  if(notification?.kind==="hot-story"&&notification?.textPolicy!=="facts-only-independent-wording-v248"){
...
async function queuePushJob(storage,notification,targetDeviceId="") {
  ...
  const normalized={
    ...
    textPolicy:String(notification?.textPolicy||"").slice(0,60)
  };
...
// V248 — bounded Durable Object retention without another Cron or another DO request.
...
const STORAGE_RETENTION_V248 = Object.freeze({
  notificationDays:14,
  pushEventDays:30,
...
async function retentionDeletePrefix(storage,prefix,cutoff,selector){
  const rows=await storage.list({prefix,limit:STORAGE_RETENTION_V248.maxPrefixRowsPerRun});
  const remove=[];
  for(const [key,row] of rows.entries()){
...
async function compactOldAnalyticsDays(storage,now){
  ...
  const deleteBefore=now-STORAGE_RETENTION_V248.analyticsDayDays*86400000;
  const compactBefore=now-STORAGE_RETENTION_V248.analyticsUniqueHashDays*86400000;
...
async function runDailyStorageRetentionV248(storage,now=Date.now()){
  ...
  result.invalidSubscriptions=await retentionDeletePrefix(storage,"push.invalid:",days(STORAGE_RETENTION_V248.invalidSubscriptionDays));
  result.presence=await retentionDeletePrefix(storage,"presence:",days(STORAGE_RETENTION_V248.presenceDays));
  result.contactRate=await retentionDeletePrefix(storage,"contact.rate:",days(STORAGE_RETENTION_V248.contactRateDays));
  result.pushFailureScratch=await retentionDeletePrefix(storage,"push.failureScratch:",days(STORAGE_RETENTION_V248.pushFailureScratchDays));
  result.analyticsHours=await retentionDeletePrefix(storage,"analytics.hour:",days(STORAGE_RETENTION_V248.analyticsHourDays),(key,row)=>retentionDateFromKey(key,"analytics.hour:")||retentionRowTime(row,0));
...
async function maybeRunDailyStorageRetentionV248(storage,now=Date.now()){
  ...
  async fetch(request){
    ...
    if(url.pathname==="/collect-news-shard"&&request.method==="POST"){
      ...
      try{
        ...
        // V237: successful collector.last was never read by runtime/admin code and
        // wrote six diagnostic rows every minute. Keep failure diagnostics below,
        // but avoid the success-only Storage write on the hot path.
        if(data?.rawPayload===true){
          return new Response(JSON.stringify(payload),{status:200,headers:{"Content-Type":"application/json; charset=utf-8","Cache-Control":"no-store","X-KP-Generated-At":String(payload.generatedAt||""),"X-KP-Items":String(Array.isArray(payload.items)?payload.items.length:0),"X-KP-Stale":payload.stale===true?"1":"0","X-KP-Served-From-Cache":payload.servedFromCache===true?"1":"0"}});
        }
    ...
    if(url.pathname==="/hot-story/display"&&request.method==="POST"){
      ...
      const row={fingerprint:String(data?.fingerprint||"").slice(0,220),title:cleanPushTitle(data?.title||""),sources:Math.max(0,Math.min(100,Number(data?.sources)||0)),at:data?.at||new Date().toISOString(),link:cleanHotUrl(data?.link),image:cleanHotUrl(data?.image),receivedAt:new Date().toISOString(),source:"live-site",textPolicy:"facts-only-independent-wording-v248"};
    ...
    if(url.pathname==="/hot-story"){
      ...
      if(display&&Number.isFinite(displayAge)&&displayAge<=3*60*1000){
        const safeDisplay=display?.textPolicy==="facts-only-independent-wording-v248"?display:{...display,title:"סיפור מרכזי חדש בכותרת פלוס",image:"",textPolicy:"legacy-hidden-after-v241"};
        return json({...safeDisplay,syncedAt:display.receivedAt},200,{"Cache-Control":"no-store","X-Koteret-Text-Policy":"facts-only-independent-wording-v248"});
      }
      ...
      if(latest&&age<=10*60*1000){
        const safeLatest=latest?.textPolicy==="facts-only-independent-wording-v248"?latest:{...latest,title:"סיפור מרכזי חדש בכותרת פלוס",image:"",textPolicy:"legacy-hidden-after-v241"};
        return json({...safeLatest,syncedAt:latest.generatedAt||latest.receivedAt||latest.at,source:"background"},200,{"Cache-Control":"no-store","X-Koteret-Text-Policy":"facts-only-independent-wording-v248"});
      }
    ...
    if(url.pathname==="/latest"){
      ...
      const primary=displayLatest||latest;
      ...
      const fullIsPublicSafe=full?.textPolicy==="facts-only-independent-wording-v248";
      ...
      const safePrimary=primary?.textPolicy==="facts-only-independent-wording-v248"?primary:{...primary,title:"סיפור מרכזי חדש בכותרת פלוס",textPolicy:"legacy-hidden-after-v241"};
      const safeLatest=latest?.textPolicy==="facts-only-independent-wording-v248"?latest:(latest?{...latest,title:"סיפור מרכזי חדש בכותרת פלוס",textPolicy:"legacy-hidden-after-v241"}:null);
      return json({...safePrimary,displayEntry:matchingFull,pushLatest:safeLatest},200,{"Cache-Control":"no-store","X-Koteret-Text-Policy":"facts-only-independent-wording-v248"});
    }
    ...
    if(url.pathname==="/notification"){
      ...
      return notification?json(notification,200,{"Cache-Control":"no-store","X-Koteret-Text-Policy":"facts-only-independent-wording-v248"}):json({error:"No notification yet"},404,{"Cache-Control":"no-store"});
    }
    ...
    if(url.pathname==="/analytics/visit"&&request.method==="POST"){
      ...
      const day=await storage.get(dayKey)||{date:parts.date,views:0,pages:{home:0,escalation:0,info:0,other:0},unique:0,uniqueHome:0,uniqueEscalation:0,uniqueHashes:[],homeUniqueHashes:[],escalationUniqueHashes:[],devices:{mobile:0,tablet:0,desktop:0},sources:{direct:0,google:0,meta:0,x:0,whatsapp:0,share:0,internal:0,other:0},referrerDomains:{}};
      ...
      const source=analyticsReferrerGroup(data?.referrer||"",data?.sourceHint||"");day.sources={direct:0,google:0,meta:0,x:0,whatsapp:0,share:0,internal:0,other:0,...(day.sources||{})};day.sources[source]=Number(day.sources[source]||0)+1;
      const refDomain=analyticsReferrerDomain(data?.referrer||"");if(refDomain&&!/koteretplus|singles-pr\.workers\.dev/.test(refDomain)){day.referrerDomains={...(day.referrerDomains||{})};day.referrerDomains[refDomain]=Number(day.referrerDomains[refDomain]||0)+1;day.referrerDomains=analyticsTrimCounts(day.referrerDomains,30);}
    ...
    if(url.pathname==="/admin/session/create"&&request.method==="POST"){
      const data=await request.json().catch(()=>({}));
    ...
    if(url.pathname==="/admin/session/verify"&&request.method==="POST"){
      const data=await request.json().catch(()=>({})),tokenHash=String(data?.tokenHash||"").replace(/[^a-zA-Z0-9_-]/g,"").slice(0,120);
      if(!tokenHash)return json({ok:false},401,{"Cache-Control":"no-store"});
      const key=`admin.session:${tokenHash}`,row=await storage.get(key);
      if(!row)return json({ok:false},401,{"Cache-Control":"no-store"});
      const expiresAt=Date.parse(row.expiresAt||0);if(!Number.isFinite(expiresAt)||expiresAt<=Date.now()){await storage.delete(key);return json({ok:false},401,{"Cache-Control":"no-store"})}
      const lastSeenAt=Date.parse(row.lastSeenAt||0);if(!Number.isFinite(lastSeenAt)||Date.now()-lastSeenAt>60*60*1000)await storage.put(key,{...row,lastSeenAt:new Date().toISOString()});
      return json({ok:true},200,{"Cache-Control":"no-store"});
    }
    if(url.pathname==="/admin/session/delete"&&request.method==="POST"){
      const data=await request.json().catch(()=>({})),tokenHash=String(data?.tokenHash||"").replace(/[^a-zA-Z0-9_-]/g,"").slice(0,120);
      if(tokenHash)await storage.delete(`admin.session:${tokenHash}`);
      return json({ok:true},200,{"Cache-Control":"no-store"});
    }
    ...
    if(url.pathname==="/background-heartbeat"&&request.method==="POST"){
      ...
      await maybeRunDailyStorageRetentionV248(storage);
    ...
    if(url.pathname==="/background-news"&&request.method==="POST"){
      ...
      for(const shard of ESCALATION_SHARDS){
        ...
        const row={payload,storedAt:new Date(now).toISOString()};
        await storage.put(`push.newsShard:${shard}`,row);
        incomingRows.set(shard,row);
        storedCount++;
      }
      ...
      const shardAges={};
      for(const shard of ESCALATION_SHARDS){
        // V237: a shard successfully PUT above is already available in this same
        // invocation. Reuse it directly and only hit Storage for missing shards,
        // preserving the existing fallback/stale-delete behavior byte-for-byte.
        const row=incomingRows.has(shard)?incomingRows.get(shard):await storage.get(`push.newsShard:${shard}`);
      ...
      // V217: keep the strict V214 selector, but align autonomous Push with the
      // actual public hero when the hero is a NEW, fresh, multi-source editorial
      // choice. This avoids the contradictory state where the site declares a
      // story "the main story now" yet subscribers remain silent for hours.
      const strictPushEntry=clustered.length?selectServerPushLead(clustered,now):null;
      let displayEntry=clustered.length?selectServerDisplayLead(clustered,now,strictPushEntry):null;
      const lastPushedStateForBridge=await storage.get("lead.lastPushedState");
      const displayPushBridge=serverDisplayLeadPushBridge(displayEntry,lastPushedStateForBridge,now);
      const displayDiffersFromStrict=!!displayEntry&&(!strictPushEntry||String(displayEntry.fingerprint||"")!==String(strictPushEntry.fingerprint||""));
      const safetyNet=clustered.length?serverPushSafetyEntry(clustered,lastPushedStateForBridge,now):{entry:null,reason:"no-news",minutesSinceLastPush:displayPushBridge?.minutesSinceLastPush};
      const strictProbe=strictPushEntry?serverLeadPayload(strictPushEntry):null;
      const strictSameAsLast=!!strictProbe&&!!lastPushedStateForBridge?.fingerprint&&leadPushSameStory(lastPushedStateForBridge,strictProbe);
      let notificationEntry=displayDiffersFromStrict&&displayPushBridge.eligible?displayEntry:(strictPushEntry&&!strictSameAsLast?strictPushEntry:null);
      let pushSelectionMode=displayDiffersFromStrict&&displayPushBridge.eligible?"display-main-headline":notificationEntry?"strict":"";
      let pushSelectionReason=displayDiffersFromStrict&&displayPushBridge.eligible?displayPushBridge.reason:notificationEntry?"strict-qualified":"";
      // V217 safety net: after prolonged silence, a DIFFERENT credible 2+ source
      // story may become both the public hero and Push candidate. An old strict
      // winner that is merely the SAME already-pushed story must not block it.
      if(!notificationEntry&&safetyNet.entry){
        notificationEntry=safetyNet.entry;
        displayEntry=safetyNet.entry;
        pushSelectionMode="silence-safety-net";
        pushSelectionReason=safetyNet.reason;
      }
      // If there is no different eligible story, continue feeding the strict
      // current story to /lead so legitimate material updates can still trigger
      // the existing story-update path. Dedupe prevents a repeat otherwise.
      if(!notificationEntry&&strictPushEntry){
        notificationEntry=strictPushEntry;
        pushSelectionMode="strict-existing-story";
        pushSelectionReason="same-strict-story-update-check";
      }
      const payload=serverLeadPayload(notificationEntry);
      if(payload){
        payload.pushSelectionMode=pushSelectionMode||"strict";
        payload.pushSelectionReason=pushSelectionReason||"strict-qualified";
      }
      const topCandidates=serverPushCandidateDiagnostics(clustered,lastPushedStateForBridge,notificationEntry,strictPushEntry,now);
      let displaySnapshot=serverLeadDisplaySnapshot(displayEntry);
      let displaySummary=displayEntry?serverLeadDisplaySummary(displayEntry,now,!!notificationEntry&&String(displayEntry.fingerprint||"")===String(notificationEntry.fingerprint||"")):null;
      ...
      if(!displaySnapshot){
        ...
        if(recoveredEntry){
          recoveredEntry.displayReason="verified-recovery";
          displayEntry=recoveredEntry;
          displaySnapshot=serverLeadDisplaySnapshot(recoveredEntry);
          displaySummary=serverLeadDisplaySummary(recoveredEntry,now,false);
          displayRecovered=!!displaySnapshot;
        }
      }
      if(displaySnapshot){
        await storage.put("lead.fullDisplay",displaySnapshot);
        if(displaySummary)await storage.put("lead.displayLatest",displaySummary);
      }
    ...
    if(url.pathname==="/lead"&&request.method==="POST"){
      ...
      const recentPushedRows=recentLeadPushes(await storage.get("lead.recentPushedStories"));
      const recentSameEvent=[...recentPushedRows].reverse().find((row)=>leadPushSameStory(row,payload))||null;
      const samePushedStory=!!lastState&&leadPushSameStory(lastState,payload);
      const sameObservedOnly=leadPushSameStory(observed,payload);
      // V220: if the current cluster drifts in wording/fingerprint while sources
      // accumulate, the last delivered event ledger remains authoritative. This
      // prevents the same real-world event from being mistaken for a new story.
      const sameObservedStory=sameObservedOnly||samePushedStory||!!recentSameEvent;
      // Only a state that actually belongs to THIS event may authorize an update
      // Push. Never compare against an unrelated globally-last pushed story.
      const sameEventState=samePushedStory?lastState:(recentSameEvent||null);
      const observedFingerprintChanged=String(observed.fingerprint||"")!==String(payload.fingerprint||"");
      let changeKind="";
      let changeReason="";
      let titleDelta=null;
      let materialDevelopment=null;
      ...
      }else if(sameEventState){
        // V220: SAME EVENT = one Push. Growing source count, a new representative
        // headline, image change or ordinary rewrite do not qualify. A second
        // Push is reserved for a concrete factual development only.
        materialDevelopment=leadPushMaterialDevelopment(sameEventState,payload);
        titleDelta=materialDevelopment.titleDelta;
        const lastAt=Date.parse(sameEventState.latestAt||sameEventState.at||0);
        const nextAt=Date.parse(payload.at||0);
        const newReport=Number.isFinite(nextAt)&&(!Number.isFinite(lastAt)||nextAt>lastAt+30*1000);
        const sameMaterialSignature=String(sameEventState.materialSignature||"")&&String(sameEventState.materialSignature||"")===String(materialDevelopment.signature||"");
        if(materialDevelopment.material&&newReport&&!sameMaterialSignature){
          const pushedAt=Date.parse(sameEventState.pushedAt||0);
          const sincePush=Number.isFinite(pushedAt)?now-pushedAt:Infinity;
          const minGap=60*1000;
          if(sincePush>=minGap){changeKind="story-update";changeReason="factual-development";}
          else changeReason="update-cooldown";
        }else if(!newReport&&materialDevelopment.material)changeReason="no-new-report";
        else if(sameMaterialSignature)changeReason="same-material-development";
        else if(titleDelta.changed)changeReason="same-event-rewrite-or-source-growth";
      ...
      if(changeKind){
        ...
        if(!duplicate){
          ...
          const basisState=sameObservedStory?(sameEventState||lastState):null;
          const eventId=leadPushEventId(payload,basisState);
          const currentMaterial=materialDevelopment||leadPushMaterialDevelopment(basisState||{},payload);
          const updateSuffix=changeKind==="story-update"?`:${pushIdentityHash(currentMaterial.signature||pushTitleIdentity(payload.title))}`:"";
          // V220 uses the stable event id for a new-story Push and a factual
          ...
          queued=await queuePushJob(storage,{fingerprint:notificationFingerprint,kind:"hot-story",title:`הסיפור המרכזי · ${sourceCount} מקורות`,body:`🔥 ${cleanPushTitle(payload.title)}`,url:"/",at:payload.at,sources:sourceCount,changeKind,textPolicy:"facts-only-independent-wording-v248"});
          if(queued.ok){
      ...
      await storage.put("lead.lastDecision",{at:new Date().toISOString(),origin:payload.origin,fingerprint:payload.fingerprint,title:payload.title,changeKind:changeKind||null,reason:changeReason||(!changeKind?"same-story":"queued"),queued:!!queued?.queued,duplicate:!!queued?.duplicate,materialUpdate:materialDevelopment?{material:materialDevelopment.material,numbersChanged:materialDevelopment.numbersChanged,addedFamilies:materialDevelopment.addedFamilies}:null});
    ...
    if(job.targetKey){
      ...
      if(row){
        try{
          ...
          const notification=safeStoredNotificationForDelivery(await storage.get(`notification:${job.notificationFingerprint}`));
