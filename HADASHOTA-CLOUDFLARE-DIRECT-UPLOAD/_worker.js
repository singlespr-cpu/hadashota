...
export default {
  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/api/push/click") {
      if (request.method !== "POST") return json({ error: "Method not allowed" }, 405);
      return handlePushHubRequest(env, "/click", request);
    }
    ...
    if (url.pathname === "/api/alerts") {
      if (request.method === "OPTIONS") return cors(new Response(null, { status: 204 }));
      if (request.method !== "GET") return json({ error: "Method not allowed" }, 405);
      const response=await handleEmergencyAlerts(ctx);
      // Empty OREF polls are extremely frequent while the site is open. Only an
      // actual active alert is promoted to the PushHub here; the Cron performs
      // the authoritative background sync once per minute.
      try { const payload=await response.clone().json(); if(payload?.ok&&Array.isArray(payload.alerts)&&payload.alerts.length)ctx.waitUntil(handlePushHubRequest(env,"/alerts/sync",new Request("https://push.internal/alerts/sync",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({alerts:payload.alerts,checkedAt:payload.checkedAt})}))); } catch {}
      return response;
    }
    ...
    // V160: the workers.dev hostname is kept only as a technical endpoint.
...
async function handlePromoAdmin(request,env){
  ...
  if(action==="remove"){const existing=await readPromo(request);const promo={active:false,text:"",url:"",imageData:"",updatedAt:new Date().toISOString()};await writePromo(request,promo);if(existing?.id)await adminHubCall(env,"/admin/ad",{action:"delete",adId:existing.id,slot:"top"}).catch(()=>{});return json({ok:true,promo},200,{"Cache-Control":"no-store"})}
  if(action==="save"){const existing=await readPromo(request);const promo=sanitizePromoPayload(body?.promo||{});if(!promo.active)return json({error:"יש להזין מלל וקישור תקין"},400,{"Cache-Control":"no-store"});promo.id=existing?.id||crypto.randomUUID();await writePromo(request,promo);return json({ok:true,promo},200,{"Cache-Control":"no-store"})}
...
async function handleFeedPromoAdmin(request,env){
  ...
  if(action==="remove"){const existing=await readFeedPromo(request);const promo={active:false,text:"",url:"",imageData:"",updatedAt:new Date().toISOString()};await writeFeedPromo(request,promo);if(existing?.id)await adminHubCall(env,"/admin/ad",{action:"delete",adId:existing.id,slot:"feed"}).catch(()=>{});return json({ok:true,promo},200,{"Cache-Control":"no-store"})}
  if(action==="save"){const existing=await readFeedPromo(request);const promo=sanitizePromoPayload(body?.promo||{});if(!promo.active)return json({error:"יש להזין מלל וקישור תקין"},400,{"Cache-Control":"no-store"});promo.id=existing?.id||crypto.randomUUID();await writeFeedPromo(request,promo);return json({ok:true,promo},200,{"Cache-Control":"no-store"})}
  return json({error:"Unknown action"},400,{"Cache-Control":"no-store"})
}
...
async function handleEmergencyAlerts(ctx) {
  ...
  try {
    const response = await fetch(endpoint, {
      ...
      cf: { cacheEverything: true, cacheTtl: 8 }
    });
...
async function handleUtilities(request, ctx) {
  ...
  const utilityCache=caches.default,utilityKey=new Request(`https://koteretplus.com/__utilities_v160?city=${encodeURIComponent(cityKey)}`,{method:"GET"});
  const utilityHit=await utilityCache.match(utilityKey);if(utilityHit)return utilityHit;
  ...
  const utilityResponse=json({
    ...
    "X-Koteret-Utilities-Cache":"MISS"
  });
  ctx?.waitUntil(utilityCache.put(utilityKey,utilityResponse.clone()));
...
function maybeTrackOnlinePresenceFromUrl(request,env,ctx,defaultPage="home"){
  try{const u=new URL(request.url);if(u.searchParams.get("presence")!=="1")return;const deviceId=String(u.searchParams.get("device")||"").replace(/[^a-zA-Z0-9._:-]/g,"").slice(0,120);if(!deviceId)return;const page=["home","escalation","info"].includes(String(u.searchParams.get("page")))?String(u.searchParams.get("page")):defaultPage;ctx.waitUntil(handlePushHubRequest(env,"/analytics/heartbeat",new Request("https://push.internal/analytics/heartbeat",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({deviceId,page})})).catch(()=>{}))}catch{}
}

async function readNewsSnapshotForBundle(request,shard){
  const cache=caches.default;
  const currentUrl=new URL(request.url);currentUrl.pathname="/api/news";currentUrl.search=`?shard=${shard}&v=119`;
  let hit=await cache.match(new Request(currentUrl.toString(),{method:"GET"}));let source="shared";
  if(!hit){const lastUrl=new URL(request.url);lastUrl.pathname="/api/news-last-good";lastUrl.search=`?shard=${shard}&v=119`;hit=await cache.match(new Request(lastUrl.toString(),{method:"GET"}));source="last-good"}
  if(!hit)return null;
  try{const payload=await hit.json();return {...payload,servedFromCache:true,bundleSource:source}}catch{return null}
}

// V160: caches.default is POP-local and cannot be used as the only shared news
// store. Keep a compact copy of every Cron-produced shard in the existing
// Durable Object so every Cloudflare location can bootstrap from the same data.
function compactNewsItemForGlobal(item={}){
  const compact={...item};
  if(Array.isArray(compact.related))compact.related=compact.related.slice(0,8).map((row)=>({
    id:row?.id,sourceId:row?.sourceId,publisher:row?.publisher,sourceName:row?.sourceName,sourceKind:row?.sourceKind,
    verified:!!row?.verified,official:!!row?.official,independent:!!row?.independent,url:row?.url,publishedAt:row?.publishedAt,
    title:String(row?.title||"").slice(0,260),preview:String(row?.preview||row?.description||"").slice(0,320),
    imageUrl:row?.imageUrl,imageCredit:row?.imageCredit,category:row?.category
  }));
  if(Array.isArray(compact.updates))compact.updates=compact.updates.slice(-8);
  if(compact.description)compact.description=String(compact.description).slice(0,420);
  if(compact.preview)compact.preview=String(compact.preview).slice(0,340);
  return compact;
}
function compactNewsShardForGlobal(payload={}){
  const out={...payload,items:(Array.isArray(payload?.items)?payload.items:[]).slice(0,120).map(compactNewsItemForGlobal),sources:(Array.isArray(payload?.sources)?payload.sources:[]).slice(0,80),globalStoredAt:new Date().toISOString()};
  out.globalFingerprint=stableId(`${out.shard||""}|${out.items.slice(0,36).map((x)=>x?.id||x?.url||x?.title||"").join("|")}`);
  // SQLite-backed Durable Object values allow up to 2 MB. Stay comfortably
  // below that even when many headlines contain Hebrew text or rich metadata.
  try{
    while(out.items.length>24&&new TextEncoder().encode(JSON.stringify(out)).byteLength>1450000)out.items=out.items.slice(0,Math.max(24,Math.floor(out.items.length*.75)));
  }catch{}
  return out;
}
async function storeGlobalNewsShard(env,shard,payload){
  const stub=pushHubStub(env);if(!stub||!payload)return false;
  try{
    const compact=compactNewsShardForGlobal({...payload,shard});
    const r=await stub.fetch(new Request("https://push.internal/news/shard",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(compact)}));
    return r.ok;
  }catch(error){console.warn(`Global news shard store failed (${shard})`,error);return false;}
}
async function readGlobalNewsBundle(env){
  const stub=pushHubStub(env);if(!stub)return [];
  try{
    const r=await stub.fetch(new Request("https://push.internal/news/bundle",{method:"GET"}));
    if(!r.ok)return [];
    const data=await r.json();
    return Array.isArray(data?.shards)?data.shards:[];
  }catch(error){console.warn("Global news bundle read failed",error);return [];}
}

async function handleNewsBundle(request,env,ctx){
  maybeTrackOnlinePresenceFromUrl(request,env,ctx,"home");
  const cache=caches.default,bundleKey=new Request("https://koteretplus.com/__news_bundle_v160",{method:"GET"});
  const bundleHit=await cache.match(bundleKey);if(bundleHit)return cors(bundleHit);

  // 1) Prefer this POP's hot edge cache.
  let shards=(await Promise.all(ESCALATION_SHARDS.map((shard)=>readNewsSnapshotForBundle(request,shard)))).filter(Boolean);
  let bundleSource="edge";

  // 2) On a cold POP, read the Cron-produced persistent snapshot from the DO.
  if(!shards.length){shards=await readGlobalNewsBundle(env);bundleSource="global";}

  // 3) First deployment ever: bootstrap two small shards once instead of making
  // every browser fan out to all publishers. Cron fills the remaining shards.
  if(!shards.length){
    const fresh=await refreshBackgroundNewsShards(env,ctx,["sites-1","telegram-1"]);
    shards=[...fresh.values()];bundleSource="bootstrap";
  }

  if(!shards.length)return cors(json({ok:false,shards:[],error:"shared_snapshot_not_ready"},503,{"Cache-Control":"no-store","X-Hadashota-Version":"160.0.0"}));
  const response=json({ok:true,shards,generatedAt:new Date().toISOString(),bundleSource},200,{"Cache-Control":"public, max-age=0, s-maxage=15","X-Hadashota-Version":"160.0.0","X-Koteret-Bundle":"1","X-Koteret-Bundle-Source":bundleSource});
  ctx?.waitUntil(cache.put(bundleKey,response.clone()));
  return cors(response);
}
...
/* =========================================================
   ...
   ========================================================= */
const ESCALATION_MODEL_VERSION = "146.0";
const ESCALATION_WEIGHTS = Object.freeze({news:17,official:11,aviation:15,military:3,notam:7,airrisk:9,oil:9,us:8,maritime:10,nuclear:3,market:4,diplomatic:4});
const ESCALATION_LABELS = Object.freeze({news:"מודיעין חדשותי",official:"פיקוד העורף + רשמי",aviation:"תעופה אזרחית מעל איראן",military:"נוכחות צבאית גלויה",notam:"NOTAM ישראל ומרחב אווירי",airrisk:"אזהרות תעופה אזוריות",oil:"Brent",us:"עמדת ארה״ב",maritime:"הורמוז והמפרץ",nuclear:"איראן והגרעין",market:"שוק חיזוי",diplomatic:"אזהרות בינלאומיות"});
const ESCALATION_SOURCE_URLS = Object.freeze({
  news:"/", official:"https://www.oref.org.il/", aviation:"https://www.adsb.lol/", military:"https://www.adsb.lol/",
...
function scoreKoteretNews(cacheData){
  const now=Date.now(),windowMs=12*3600000,one=3600000;
  ...
  const classified=relevant.map(r=>{const text=`${r?.title||""} ${r?.summary||r?.description||""}`,age=Math.max(0,now-Date.parse(r?.publishedAt||now));return {...r,_hard:ESCALATION_HARD_RX.test(text),_soft:ESCALATION_SOFT_RX.test(text),_deesc:ESCALATION_DEESC_RX.test(text),_recent:age<=6*3600000};});
  let weighted=0,deesc=0;for(const r of classified){const decay=r._recent?1:.55;if(r._hard)weighted+=decay;else if(r._soft)weighted+=.45*decay;if(r._deesc)deesc+=decay;}
  const denom=Math.max(1,classified.length),ratio=weighted/denom,deRatio=deesc/denom;
  const current=classified.filter(r=>now-Date.parse(r.publishedAt||0)<=one).length;
  const previous=classified.filter(r=>{const age=now-Date.parse(r.publishedAt||0);return age>one&&age<=windowMs;}).length;
  const baseline=previous/11,velocity=baseline>.45?current/baseline:(current>=4?2:1);
  const publishers=new Set(classified.filter(r=>r._hard).map(r=>r.publisher||r.sourceId).filter(Boolean)).size;
  let score=8+Math.pow(Math.min(1,ratio),1.4)*64+Math.min(14,Math.max(0,velocity-1)*8)+Math.min(8,Math.max(0,publishers-2)*2)-deRatio*22;
  if(classified.length<4)score=Math.min(score,28);score=escClamp(Math.round(score));
  const reason=classified.length?`${classified.length} דיווחים אזוריים ב־12 שעות; ${current} בשעה האחרונה; ${publishers} מקורות דיווחו על אינדיקציות הסלמה.`:"לא זוהתה כרגע מסה מספקת של דיווחים אזוריים במטמון החדשות.";
  return escSignal("news",score,true,reason,{source:"45 מקורות כותרת פלוס",sourceUrl:"/",freshness:cacheData?.freshestAt?`מטמון ${new Date(cacheData.freshestAt).toLocaleTimeString("he-IL",{hour:"2-digit",minute:"2-digit",timeZone:"Asia/Jerusalem"})}`:"מטמון חדשות",stats:{relevant:classified.length,currentHour:current,hardSources:publishers,velocity:Number(velocity.toFixed(2)),windowHours:12}});
}
...
function scoreOfficialSignal(cacheData,oref,idfWeb,nscWeb){
  ...
  let score=10+Math.min(40,escalation.length*9)+Math.min(16,directHard*4)+Math.min(12,nscHard*4)-Math.min(20,deesc.length*5+directDeesc*3+nscDeesc*3);if(active){const alertFloor=(escalation.length||directHard||nscHard)?Math.min(96,80+active*3):Math.min(74,58+active*3);score=Math.max(score,alertFloor);}
...
// V160 background calibration refresh: split the external OSINT set into three
...
async function collectExternalEscalationSignalGroup(groupIndex=0){
  ...
  const [market,diplomatic,pizza]=await Promise.all([fetchPredictionMarketSignal(),fetchDiplomaticRiskSignal(),fetchPizzaExperimental()]);
  return {signals:{market,diplomatic},experimental:{pizza},updatedAt:new Date().toISOString(),group};
}
function mergeEscalationExternal(previous,fresh){
  ...
  for(const k of ["aviation","military","notam","airrisk","oil","us","maritime","nuclear","market","diplomatic"]){
...
async function handleEscalation(request,env,ctx){
  ...
  try{
    ...
    if(!claim?.claimed&&claim?.public?.latest)return json(claim.public,200,{"Cache-Control":"no-store","X-Hadashota-Version":"160.0.0"});
    if(!claim?.claimed){const p=await escalationHubCall(env,"/escalation/public");return json(p,200,{"Cache-Control":"no-store","X-Hadashota-Version":"160.0.0"});}
    ...
    const publicData=await escalationHubCall(env,"/escalation/snapshot","POST",payload);return json(publicData,200,{"Cache-Control":"no-store","X-Hadashota-Version":"160.0.0"});
  }catch(error){console.warn("Escalation refresh failed",error);try{const p=await escalationHubCall(env,"/escalation/public");return json({...p,refreshError:String(error?.message||error)},200,{"Cache-Control":"no-store","X-Hadashota-Version":"160.0.0"});}catch{return json({ok:false,error:"Escalation index temporarily unavailable"},503,{"Cache-Control":"no-store"});}}
}
...
async function buildEscalationSnapshot(storage,payload){
  ...
  const effectiveWeight=s=>Number(s.weight||0)*staleFactor(s),availableWeight=available.reduce((a,s)=>a+effectiveWeight(s),0);
  ...
  const groups={information:["news","official","us","diplomatic"],air:["aviation","notam","airrisk"],maritime:["maritime"],market:["oil","market"],nuclear:["nuclear"],military:["military"]};
  const domainScores=Object.fromEntries(Object.entries(groups).map(([g,keys])=>[g,Math.max(0,...live.filter(s=>keys.includes(s.key)).map(s=>Number(s.score)||0))]));
  const corroboratingDomains=Object.values(domainScores).filter(v=>v>=55).length;
  const multiplier=corroboratingDomains>=5?1.22:corroboratingDomains===4?1.16:corroboratingDomains===3?1.10:corroboratingDomains===2?1.05:1;raw=escClamp(raw*multiplier);
  let score=raw,calibrationReset=false;
  if(previous&&Number.isFinite(Number(previous.score))){const prev=Number(previous.score);if(previous.modelVersion!==ESCALATION_MODEL_VERSION){score=raw;calibrationReset=true;}else{const dt=Math.max(5,Math.min(3600,(now-Date.parse(previous.updatedAt||now))/1000)),half=raw>prev?60*60:8*3600,alpha=1-Math.exp(-Math.LN2*dt/half);score=prev+alpha*(raw-prev);if(raw>=88&&prev<70)score=Math.max(score,prev+4);}}
  ...
  if(calibrationReset)changes.unshift("כיול V146: המדד אותחל מחדש לפי המודל המתוקן, כדי לא להחליק ציון ישן שנמדד במשקלים ובקו בסיס שונים.");
  ...
  const latest={modelVersion:ESCALATION_MODEL_VERSION,score,rawScore:Number(raw.toFixed(1)),level:level.label,levelKey:level.key,updatedAt:new Date().toISOString(),delta6h,coverage,confidenceLabel,availableSignals:available.length,totalSignals:Object.keys(ESCALATION_WEIGHTS).length,elevatedSignals:elevated,corroboratingDomains,multiplier,domainScores,sourceHealth,calibrationReset,signals,changes:changes.slice(0,6),experimental:payload?.experimental||{},disclaimer:"מדד 0–100 של עוצמת המתיחות; אינו הסתברות למלחמה ואינו תחזית מודיעינית."};
  let _pushCandidate=null;
  if(previous&&previous.modelVersion===ESCALATION_MODEL_VERSION&&Number.isFinite(Number(previous.score))){
    const prevScore=Number(previous.score),score1h=escClosestScore(newHistory,now-60*60000),delta1h=Number((score-(Number.isFinite(score1h)?score1h:prevScore)).toFixed(1));
    const threshold=[85,70,50].find((t)=>prevScore<t&&score>=t)||0;
    const rapid=delta1h>=10&&score>=40&&corroboratingDomains>=2;
    if(threshold||rapid){_pushCandidate={fingerprint:`escalation:${threshold||"rapid"}:${Math.round(score)}:${Math.floor(now/(15*60000))}`,kind:"escalation",title:`📡 מדד ההסלמה עלה ל-${Math.round(score)}/100`,body:threshold?`${level.label} · המדד חצה את רף ${threshold}. כמה אינדיקציות ציבוריות מתחזקות במקביל.`:`${level.label} · עלייה של ${Math.round(delta1h)} נק׳ בשעה האחרונה.`,url:"/escalation",score,threshold:threshold||null,delta1h,at:new Date().toISOString()};}
  }
  await storage.put("escalation.latest",latest);await storage.put("escalation.history",newHistory);if(payload?.external){await storage.put("escalation.external",payload.external);await storage.put("escalation.external.updatedAt",payload?.externalUpdatedAt||payload.external?.updatedAt||new Date().toISOString());}await storage.delete("escalation.lock");return {ok:true,latest,history:newHistory,experimental:latest.experimental,_pushCandidate,methodology:{modelVersion:ESCALATION_MODEL_VERSION,weights:ESCALATION_WEIGHTS,refreshSeconds:60,externalRefreshMinutes:15,scoreMeaning:"intensity-not-probability",signalFamilies:Object.keys(ESCALATION_WEIGHTS).length,sourcesConfigured:SOURCES.length,corroborationThreshold:55}};
}
...
async function refreshBackgroundNewsShards(env,ctx,shards=[]){
  ...
  await Promise.all(shards.map(async(shard)=>{
    try{
      const req=new Request(`${BACKGROUND_NEWS_ORIGIN}/api/news?shard=${encodeURIComponent(shard)}&force=1`,{method:"GET",headers:{Accept:"application/json","X-Koteret-Background":"1"}});
      const response=await handleNews(req,env,ctx);
      if(!response.ok)return;
      const payload=await response.json();
      if(Array.isArray(payload?.items)){
        fresh.set(shard,payload);
        ctx?.waitUntil(storeGlobalNewsShard(env,shard,payload));
      }
    }catch(error){console.warn(`V160 background shard ${shard} failed`,error);}
  }));
  return fresh;
}
...
async function runBackgroundEscalationFromNews(env,recent=[]) {
  ...
  }catch(error){console.warn("V160 scheduled escalation monitor failed",error);}
}
...
async function runBackgroundPushMonitor(env, ctx) {
  ...
  try {
    ...
    // Keep the expensive escalation refresh on its own minute. This prevents the
    // Cron from combining dozens of news-origin fetches with the external OSINT set.
    const context=await collectServerPushContext(env,ctx,{refresh:minute%5!==0});
    const payload=serverLeadPayload(context.entry);
    ...
    try {
      const alertResponse=await handleEmergencyAlerts(ctx);
      const alertPayload=await alertResponse.clone().json().catch(()=>null);
      if(alertPayload?.ok)await stub.fetch("https://push.internal/alerts/sync",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({alerts:alertPayload.alerts||[],checkedAt:alertPayload.checkedAt})});
    ...
    console.warn("V160 scheduled push monitor failed",error);
  }
}
...
async function ensurePushStats(storage) {
  ...
  // One-time V160 repair: a browser can rotate its PushSubscription endpoint.
  ...
  stats={schema:"v152",count:cleanRows.size,platforms,updatedAt:new Date().toISOString(),deduplicated:remove.length};
...
function normalizeAlertPlace(value=""){return String(value||"").normalize("NFKD").replace(/[־–—-]/g," ").replace(/[״׳'"]/g,"").replace(/\s+/g," ").trim().toLowerCase()}
function normalizePushPreferences(value, fallback={news:true,escalation:true,alerts:true,alertAllIsrael:true,alertCities:[]}) {
  ...
  return {
    ...
    alerts: source.alerts === undefined ? fallback.alerts !== false : source.alerts !== false,
    alertAllIsrael: source.alertAllIsrael === undefined ? fallback.alertAllIsrael !== false : source.alertAllIsrael !== false,
    alertCities: Array.isArray(source.alertCities)?source.alertCities.map(x=>String(x||"").slice(0,80)).filter(Boolean).slice(0,40):(Array.isArray(fallback.alertCities)?fallback.alertCities:[])
  };
}
function pushPreferenceAllows(row, kind="manual", notification=null) {
  ...
  if(kind==="oref-alert"){
    ...
    const areas=Array.isArray(notification?.areas)?notification.areas:[];
    if(!prefs.alertCities.length||!areas.length)return false;
    return areas.some(a=>{const na=normalizeAlertPlace(a);return prefs.alertCities.some(c=>{const nc=normalizeAlertPlace(c);return na===nc||na.startsWith(nc+" ")||nc.startsWith(na+" ")})});
  }
  return true;
}
...
async function queuePushJob(storage,notification,targetDeviceId="") {
  ...
  const recent=Array.isArray(await storage.get("push.recentDedupe"))?await storage.get("push.recentDedupe"):[];
  const recentCut=Date.now()-30*60*1000;
  const keptRecent=recent.filter(x=>Date.parse(x?.at||0)>=recentCut).slice(-99);
  if(keptRecent.some(x=>x?.fingerprint===fingerprint))return {ok:true,queued:false,duplicate:true,fingerprint};
  keptRecent.push({fingerprint,at:now});await storage.put("push.recentDedupe",keptRecent);
  const normalized={
    fingerprint,
    ...
    areas:Array.isArray(notification?.areas)?notification.areas.map(x=>String(x||"").slice(0,120)).slice(0,80):undefined
  };
  ...
  if(active){
    ...
    list.push(queued);
    await storage.put("push.queue",list.slice(-12));
  } else {
  ...
  history.push({fingerprint,kind:normalized.kind,title:normalized.title,body:normalized.body,url:normalized.url,queuedAt:now,target:safeDevice?(normalized.kind==="admin-contact"?"admin-device":"device"):"all",status:active?"queued":"sending",pushed:0,clicked:0});
  const cutoff=Date.now()-30*24*60*60*1000;
  const cleanHistory=history.filter(x=>Date.parse(x?.queuedAt||x?.finishedAt||0)>=cutoff).slice(-50);
  await storage.put("push.history",cleanHistory);
...
async function finalizePushJob(storage,job) {
  ...
  const cutoff=Date.now()-30*24*60*60*1000;await storage.put("push.history",history.filter(x=>Date.parse(x?.queuedAt||x?.finishedAt||0)>=cutoff).slice(-50));
...
export class PushHub {
  ...
  async fetch(request){
    ...
    if(url.pathname==="/config"){
      ...
      const stats=await ensurePushStats(storage);
      return json({enabled:true,publicKey:keys.publicKey,subscriptions:Number(stats.count||0),platforms:stats.platforms||{},fanout:"paged-alarm",mode:"true-web-push",version:"160.0.0"},200,{"Cache-Control":"no-store"});
    }
    ...
    if(url.pathname==="/subscribe"&&request.method==="POST"){
      ...
      await storage.put(key,{subscription,deviceId,platform,userAgent,preferences,createdAt:existing?.createdAt||now,lastSeenAt:now,pendingFingerprint:existing?.pendingFingerprint||""});
    ...
    if(url.pathname==="/hot-story/display"&&request.method==="POST"){
      const data=await request.json().catch(()=>({}));
      const row={fingerprint:String(data?.fingerprint||"").slice(0,220),title:cleanPushTitle(data?.title||""),sources:Math.max(0,Math.min(100,Number(data?.sources)||0)),at:data?.at||new Date().toISOString(),receivedAt:new Date().toISOString(),source:"live-site"};
    ...
    if(url.pathname==="/news/shard"&&request.method==="POST"){
      const data=await request.json().catch(()=>null);
      const shard=String(data?.shard||"");
      if(!/^(sites|telegram)-[123]$/.test(shard)||!Array.isArray(data?.items))return json({error:"Invalid news shard"},400,{"Cache-Control":"no-store"});
      const key=`news.shard:${shard}`,existing=await storage.get(key),now=Date.now();
      const existingAt=Date.parse(existing?.globalStoredAt||existing?.generatedAt||0);
      const unchanged=existing?.globalFingerprint&&existing.globalFingerprint===data.globalFingerprint;
      if(unchanged&&Number.isFinite(existingAt)&&now-existingAt<4*60*1000)return json({ok:true,stored:false,unchanged:true,shard},200,{"Cache-Control":"no-store"});
      const row={...data,globalStoredAt:new Date().toISOString()};
      await storage.put(key,row);
      await storage.put("news.global.updatedAt",row.globalStoredAt);
      return json({ok:true,stored:true,shard,items:row.items.length},200,{"Cache-Control":"no-store"});
    }

    if(url.pathname==="/news/bundle"&&request.method==="GET"){
      const now=Date.now(),shards=[];
      for(const shard of ESCALATION_SHARDS){
        const row=await storage.get(`news.shard:${shard}`);
        if(!row||!Array.isArray(row.items)||!row.items.length)continue;
        const generated=Date.parse(row.generatedAt||row.globalStoredAt||0);
        if(!Number.isFinite(generated)||now-generated>2*60*60*1000)continue;
        shards.push({...row,servedFromCache:true,bundleSource:"global-do"});
      }
      return json({ok:shards.length>0,shards,updatedAt:await storage.get("news.global.updatedAt")||null},shards.length?200:404,{"Cache-Control":"no-store"});
    }
    ...
    const touchOnlinePresence=async(data={})=>{const deviceId=String(data?.deviceId||"").replace(/[^a-zA-Z0-9._:-]/g,"").slice(0,120);if(!deviceId)return;const page=["home","escalation","info"].includes(String(data?.page))?String(data.page):"other";await storage.put(`online:${deviceId}`,{deviceId,page,lastSeenAt:Date.now()})};

    if(url.pathname==="/analytics/heartbeat"&&request.method==="POST"){const data=await request.json().catch(()=>({}));await touchOnlinePresence(data);return json({ok:true},200,{"Cache-Control":"no-store"});}

    if(url.pathname==="/analytics/visit"&&request.method==="POST"){
      const data=await request.json().catch(()=>({}));
      await touchOnlinePresence(data);
    ...
    if(url.pathname==="/admin/dashboard"&&request.method==="POST"){
      ...
      const adStatsRows=[...(await storage.list({prefix:"ad.stats:"})).values()];
      const ads={};for(const row of adStatsRows){if(row?.adId)ads[row.adId]=row;}
      const onlineEntries=[...(await storage.list({prefix:"online:",limit:500})).entries()],onlineCutoff=Date.now()-150000,cleanupCutoff=Date.now()-15*60000;let onlineNow=0;const onlinePages={home:0,escalation:0,info:0,other:0},staleOnlineKeys=[];for(const [key,row] of onlineEntries){const seen=Number(row?.lastSeenAt||0);if(seen>=onlineCutoff){onlineNow+=1;const pg=["home","escalation","info"].includes(String(row?.page))?String(row.page):"other";onlinePages[pg]=Number(onlinePages[pg]||0)+1}else if(seen<cleanupCutoff)staleOnlineKeys.push(key)}if(staleOnlineKeys.length)await Promise.all(staleOnlineKeys.map((key)=>storage.delete(key)));
      return json({ok:true,version:"160.0.0",analytics:{summary,days:dayRows,hours:hourRows,hourOfDay,peakHour,peakDay,today,onlineNow,onlinePages,onlineWindowSeconds:150},push:{subscriptions:Number(stats.count||0),platforms:stats.platforms||{},lastResult:lastResult||null,activeJob:activeJob||null,latestNotification:latestNotification||null,latestLead:latestLead||null,leadCandidate:leadCandidate||null,lastPushedFingerprint:lastPushedFingerprint||null,history:history.slice(-50).reverse(),adminDevices:{registered:adminDeviceRows.length,pushReady:adminPushReady}},escalation:escalation?{score:escalation.score,level:escalation.level,updatedAt:escalation.updatedAt,delta6h:escalation.delta6h,sourceHealth:escalation.sourceHealth,coverage:escalation.coverage}:null,contacts:{total:Number(contactSummary.total||0),newCount:Number(contactSummary.newCount||0),items:contactRows},ads},200,{"Cache-Control":"no-store"});
    }
    ...
    if(url.pathname==="/click"&&request.method==="POST"){
      const data=await request.json().catch(()=>({}));const fingerprint=String(data?.fingerprint||"").slice(0,220);if(!fingerprint)return json({ok:false},400,{"Cache-Control":"no-store"});
      const deviceId=String(data?.deviceId||"").slice(0,120),clickKey=`push.click:${await sha256Base64Url(fingerprint+":"+deviceId)}`;if(deviceId&&await storage.get(clickKey))return json({ok:true,duplicate:true},200,{"Cache-Control":"no-store"});
      if(deviceId)await storage.put(clickKey,{at:new Date().toISOString()});
      const history=Array.isArray(await storage.get("push.history"))?await storage.get("push.history"):[];const idx=history.map(x=>x.fingerprint).lastIndexOf(fingerprint);if(idx>=0){history[idx]={...history[idx],clicked:Number(history[idx].clicked||0)+1,lastClickedAt:new Date().toISOString()};await storage.put("push.history",history.slice(-50));}
      return json({ok:true},200,{"Cache-Control":"no-store"});
    }
    if(url.pathname==="/ad/event"&&request.method==="POST"){
      const data=await request.json().catch(()=>({})),adId=String(data?.adId||"").replace(/[^a-zA-Z0-9._:-]/g,"").slice(0,120),slot=["top","feed"].includes(String(data?.slot))?String(data.slot):"unknown",event=String(data?.event||"");if(!adId||!["view","click"].includes(event))return json({ok:false},400,{"Cache-Control":"no-store"});
      const key=`ad.stats:${adId}`,row=await storage.get(key)||{adId,slot,views:0,clicks:0,createdAt:new Date().toISOString()};row.slot=slot;row[event==="view"?"views":"clicks"]=Number(row[event==="view"?"views":"clicks"]||0)+1;row.updatedAt=new Date().toISOString();await storage.put(key,row);return json({ok:true},200,{"Cache-Control":"no-store"});
    }
    if(url.pathname==="/admin/ad"&&request.method==="POST"){
      const data=await request.json().catch(()=>({})),adId=String(data?.adId||"").replace(/[^a-zA-Z0-9._:-]/g,"").slice(0,120);if(data?.action==="delete"&&adId)await storage.delete(`ad.stats:${adId}`);return json({ok:true},200,{"Cache-Control":"no-store"});
    }
    if(url.pathname==="/alerts/sync"&&request.method==="POST"){
      const data=await request.json().catch(()=>({})),alerts=Array.isArray(data?.alerts)?data.alerts:[],now=Date.now();let seen=await storage.get("oref.push.seen")||{};for(const [k,v] of Object.entries(seen))if(now-Date.parse(v||0)>24*60*60*1000)delete seen[k];let queued=0;
      for(const alert of alerts){const areas=Array.isArray(alert?.areas)?alert.areas:[],raw=[alert?.id,alert?.category,alert?.title,...areas].join("|");const fp=`oref:${await sha256Base64Url(raw)}`;if(seen[fp])continue;seen[fp]=new Date().toISOString();const body=areas.length?areas.slice(0,8).join(" · "):(alert?.description||"התרעה חדשה");const result=await queuePushJob(storage,{fingerprint:fp,kind:"oref-alert",title:`🚨 ${cleanPushTitle(alert?.title||"התרעת פיקוד העורף")}`,body,url:"/",areas,at:data?.checkedAt||new Date().toISOString()});if(result?.queued)queued++;}
      await storage.put("oref.push.seen",seen);return json({ok:true,queued},200,{"Cache-Control":"no-store"});
    }
    ...
    if(url.pathname==="/status"){
      ...
      return json({enabled:true,subscriptions:Number(stats.count||0),platforms:stats.platforms||{},lastPushedFingerprint:previous||null,latest:latest||null,fanoutActive:!!activeJob,lastResult:lastResult||null,version:"160.0.0"},200,{"Cache-Control":"no-store"});
    }
    ...
    if(url.pathname==="/escalation/public"&&request.method==="GET") {
      ...
      if(!latest)return json({ok:false,latest:null,history,methodology:{modelVersion:ESCALATION_MODEL_VERSION,weights:ESCALATION_WEIGHTS,refreshSeconds:60,externalRefreshMinutes:15,scoreMeaning:"intensity-not-probability",signalFamilies:Object.keys(ESCALATION_WEIGHTS).length,sourcesConfigured:SOURCES.length,corroborationThreshold:55}},200,{"Cache-Control":"no-store"});
    ...
    if(url.pathname==="/lead"&&request.method==="POST"){
      ...
      const titleKey=cleanPushTitle(payload.title).toLowerCase().replace(/[^\p{L}\p{N}]+/gu," ").trim();
      const last=await storage.get("lead.lastPushedFingerprint"),lastTitleKey=await storage.get("lead.lastPushedTitleKey");
      if(!last){await storage.put("lead.lastPushedFingerprint",payload.fingerprint);await storage.put("lead.lastPushedTitleKey",titleKey);await storage.delete("lead.candidate");return json({ok:true,primed:true,pushed:0});}
      if(last===payload.fingerprint||lastTitleKey===titleKey){await storage.delete("lead.candidate");return json({ok:true,changed:false,pushed:0,duplicate:true});}
      ...
      if(queued.ok&&!queued.duplicate){await storage.put("lead.lastPushedFingerprint",payload.fingerprint);await storage.put("lead.lastPushedTitleKey",titleKey);await storage.delete("lead.candidate");}
  ...
  async alarm(){
    ...
    if(job.targetKey){
      ...
      if(row){
        try{
          ...
          const result=await sendEmptyWebPush(updated.subscription,keys);job.processed=1;
    ...
    for(let i=0;i<rows.length;i+=5){
      ...
      const results=await Promise.all(eligible.map(async([key,row])=>{
        try{const result=await sendEmptyWebPush(row?.subscription,keys);if(result.ok)return {key,ok:true};if([404,410].includes(result.status))return {key,remove:true,row};return {key,ok:false,status:result.status};}
