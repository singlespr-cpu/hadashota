...
async function handleEmergencyAlerts(ctx) {
  ...
  const cacheKey = new Request("https://hadashota.internal/v77/oref-current", { method: "GET" });
...
async function findCommonsMedia(query, specificity = 1) {
  ...
  try {
    const res = await fetch(api.toString(), { headers: { "Accept": "application/json", "User-Agent": "Hadashota/77 (+strict semantic media resolver)" } });
...
async function findOpenverseMedia(query, specificity = 1) {
  ...
  try {
    const res = await fetch(searchUrl.toString(), { headers: { "Accept": "application/json", "User-Agent": "Hadashota/77 (+news aggregator; strict semantic media lookup)" } });
    if (!res.ok) return null;
    const data = await res.json();
