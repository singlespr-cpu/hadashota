...
async function handleEscalation(request,env,ctx){
  try{
    ...
    if(!claim?.claimed&&claim?.public?.latest)return json(claim.public,200,{"Cache-Control":"no-store","X-Hadashota-Version":"182.0.0"});
    if(!claim?.claimed){const p=await escalationHubCall(env,"/escalation/public");return json(p,200,{"Cache-Control":"no-store","X-Hadashota-Version":"182.0.0"});}
    ...
    const publicData=await escalationHubCall(env,"/escalation/snapshot","POST",payload);return json(publicData,200,{"Cache-Control":"no-store","X-Hadashota-Version":"182.0.0"});
  }catch(error){console.warn("Escalation refresh failed",error);try{const p=await escalationHubCall(env,"/escalation/public");return json({...p,refreshError:String(error?.message||error)},200,{"Cache-Control":"no-store","X-Hadashota-Version":"182.0.0"});}catch{return json({ok:false,error:"Escalation index temporarily unavailable"},503,{"Cache-Control":"no-store"});}}
}
...
function compactPushPayload(notification={}) {
  return JSON.stringify({
    fingerprint:String(notification?.fingerprint||"").slice(0,220),
    kind:String(notification?.kind||"push").slice(0,30),
...
async function sendWebPush(subscription,keys,notification) {
  const endpoint=String(subscription?.endpoint||"");
  if(!/^https:\/\//i.test(endpoint))return {ok:false,status:400,mode:"payload"};
  try{
    const authorization=await vapidAuthorization(endpoint,keys);
    const body=await encryptWebPushPayload(subscription,compactPushPayload(notification));
    const response=await fetch(endpoint,{method:"POST",headers:{Authorization:authorization,TTL:"120",Urgency:"high","Content-Encoding":"aes128gcm","Content-Type":"application/octet-stream"},body});
    return {ok:response.ok,status:response.status,mode:"payload"};
  }catch(error){
    console.warn("Encrypted Web Push failed; using empty-push fallback",String(error?.message||error));
    return sendEmptyWebPush(subscription,keys);
  }
}
...
export class PushHub {
  ...
  async fetch(request){
    ...
    if(url.pathname==="/config"){
      ...
      const stats=await ensurePushStats(storage);
      return json({enabled:true,publicKey:keys.publicKey,subscriptions:Number(stats.count||0),platforms:stats.platforms||{},fanout:"paged-alarm",mode:"true-web-push",version:"182.0.0"},200,{"Cache-Control":"no-store"});
    }
    ...
    if(url.pathname==="/subscribe"&&request.method==="POST"){
      ...
      // V182: both / and /escalation share hadashota.pushDeviceId.  Older
    ...
    if(url.pathname==="/status"){
      ...
      return json({enabled:true,subscriptions:Number(stats.count||0),platforms:stats.platforms||{},lastPushedFingerprint:previous||null,latest:latest||null,fanoutActive:!!activeJob,lastResult:lastResult||null,version:"182.0.0"},200,{"Cache-Control":"no-store"});
    }
  ...
  async alarm(){
    ...
    for(let i=0;i<rows.length;i+=5){
      ...
      for(const [key,row] of batch){
        // V182 server-side device dedupe: only the subscription currently mapped
